...
| ship gate   | the commit gate, plus `npm run perf:check`; CI runs the WebKit lane on every PR and the release PR can't merge past it ([testing.md § The WebKit lane](docs/contributing/testing.md#the-webkit-lane)) | before a merge or a release; several minutes on top                              |
