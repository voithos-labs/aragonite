...
Markdown has no syntax for "a box with a name on it". No callout, no admonition, no embed, nothing. The `:::name` family is what the community settled on instead, and aragonite implements it: a fenced construct with a **name** the editor dispatches on.
...
A plugin claims the name `note` and gets a real block kind for it, with its own component, keymap, and commands. Any name **nobody** claims still parses, renders as a plain labelled box, and serializes back byte-for-byte. That second half is the whole point: **a document written with your plugin survives being opened without it.**

This guide is for plugin authors, and it assumes you have already read the [plugin author guide](plugin-guide.md). Directives replace the parsing half of a plugin, not the registration machinery underneath it.
...
A single shared opener owns every `:::` / `::` / `:` fence and dispatches on the name. N plugins therefore never fight over opener priority.

_Why not just let every plugin register its own opener?_ Because the first one to register greedily claims _every_ `:::xxx` fence, and no second plugin can ever own its own name. That is the classic way this feature gets built wrong.
...
A container closes on the first line that is a colon run **at least as long** as its opener and nothing else. It is the same length rule fenced code blocks apply to backticks. A bare `:::` inside a 3-colon container's body closes it early, so nesting a `:::` container means the outer fence must be longer:
...
**Registration is once, and a duplicate throws.** That is the `customElements` model the schema registries follow. There is no unregister and no silent override.
...
Everything after the name on the opener line is a **verbatim `info` string**, captured including its leading separator. The info is the **round-trip truth**, and it is never re-parsed to reconstruct bytes.
...
**Known limitation:** the helper is `info → structure` only. There is no inverse, sorry. A directive that _edits_ attributes rewrites its info string itself, through metadata plus its `rebuildRaw`, which is the proven container path. An inverse is an additive follow-on if a consumer needs one.
...
`ParsedDirective` is the one your `fromDirective` actually receives, so it is worth knowing without opening the `.d.ts`:

| Field                  | What it holds                                                                                     |
| ---------------------- | ------------------------------------------------------------------------------------------------- |
| `fence`                | the opener as a `DirectiveFence`: `tier`, `colonCount` (2 = leaf, 3+ = container), `name`, `info` |
| `body`                 | the parsed inner document, absent on a leaf. Its `children` become your node's body               |
| `leadingTrivia`, `raw` | pass both straight through, or the node stops serializing to what it was parsed from              |
| `closerColonCount`     | the closing fence's length, which need not match the opener's                                     |
| `closerNewline`        | whether the closer ended with a newline                                                           |
| `lineEnding`           | the opener's line ending, so a rebuild reproduces CRLF instead of quietly normalising it          |

The last four are not decoration. `createDirectiveRebuild` is generic over a metadata type that **must** carry `colonCount`, `closerColonCount`, `closerNewline` and `lineEnding` under exactly those names, because those four are what let it re-emit a fence byte-identical to the one it parsed. The compiler enforces it, so you find out immediately rather than in a round-trip failure, but it is easier to know than to discover.
