...
This is everything you need to teach the editor your own block or inline content. The whole authoring surface sits on one import path, the `@voithos-labs/aragonite/plugin` subpath, deliberately separate from the `@voithos-labs/aragonite` embedding barrel a consumer imports. One of those is for people using the editor. This one is for people extending it.

Start here. The `:::name` directive grammar gets its own [directives guide](directives.md); embedding, theming, and events belong to the [consumer guide](consumer-guide.md). And every export named anywhere below is cataloged in the [API reference](plugin-api.md), so you never have to guess whether a name is real.
...
Only the kind and its descriptor are always required. The component is what makes it visible, the grammar is what makes it parseable from Markdown. Skip the component and your kind renders a visible raw fallback; skip the descriptor and you get an error at first use.

**Registration is process-global and register-once.** A kind is a definition every editor on the page shares, which is the `customElements` model: `customElements.define` defines an element for every document, not for one. Register the same kind, component, or opener twice and you get a **conflict that throws**, never a silent override, so a plugin colliding with a built-in or with another plugin fails loudly instead of quietly winning. There is no unregister and no runtime replace. (Under a dev server, re-evaluating a registration module replaces its prior registrations in place, so a changed definition takes effect on re-run; editing a plugin unit's own `definePlugin` still needs a page reload. That replace covers **every** register-once seam, the paste-transform registry included, while production and a test run keep the throw, so the throw is what your suite observes.)

Registrations get packaged into a **plugin unit** whose `setup` runs at most once per process. So write each `register*` call straight and let the unit, not a per-call guard, own idempotence. A registrar that runs at **module scope** has no such owner, and guards each call on a probe instead: `isBlockKindDeclared`, `isBlockKindRegistered`, `isBlockComponentRegistered`, `isBlockOpenerRegistered`, `isBlockCompleterRegistered`, `isPasteTransformRegistered`, `isDirectiveRegistered`, and `isInlineKindDeclared` for the inline tier. Guard on the probe, never on a module-level `registered` flag. A flag survives `resetPluginPlatformForTests()` and then silently skips the re-registration your next test case needed, which is a fun half hour to spend.

### The plugin unit

`definePlugin({ name, setup })` validates a unit at definition time and returns an `EditorPlugin`; its `setup` runs your `register*` calls. By convention a plugin is a **factory export**, meaning `export function myPlugin(deps?)` returns the unit, so a **process-global dependency** the plugin needs (a render engine, say, which is the same for every editor) rides the factory's argument. Per-_instance_ configuration takes a different path ([Per-instance context](#per-instance-context)). The factory argument is for what never varies between editors, and only that.
...
**A plugin installs once per process, keyed by name.** Passing the same unit again no-ops. Passing a _different_ unit under a name already installed keeps the first and dev-warns (naming the loser as `name@version` when it carries one). Units install in array order, and a `setup` that throws stays failed: a later attempt rethrows and tells you to reload, because a partial setup cannot re-run against the register-once registries.

Definitions are process-global, so two editors passing the same plugin share one registration. Per-instance _configuration_ is not: an editor may pass `{ plugin, options }` and the plugin reads its own `options` off each instance's context ([Per-instance context](#per-instance-context)). Reserve the factory argument for a process-global dependency, and route anything two editors would ever want to differ on through the prop's options.

For an editor-less `parse()` pipeline that needs the grammar live without mounting `<Editor>`, call `installPlugins(units)` from the `@voithos-labs/aragonite` barrel, with the same once-per-process semantics. `isPluginInstalled(name)` probes an install for the rare setup that has to branch on it; the prop and `installPlugins` are already idempotent, so most consumers never reach for it.
...
There is no plugin-state field, and you do not need one. Keep your own map keyed on `editorId`, seed it when the editor mounts, recompute on the `edit` event, and delete the entry in the disposer. That is the whole feature.
...
Two editors share one process-global registration but may still want different options, which is the split-pane case. A consumer varies them per editor through the `plugins` prop's entry form:
...
Every surface that hands a plugin a node to **read** types it as a view: `NodeView` for a block node, `DocumentView` for the root. A view is deep-readonly on the serialized bytes (`raw`, `kind`, `metadata`, trivia, children structure), so "never mutate the tree from the view layer" is not a rule you have to remember. A byte write through a view is a compile error, not a dev-mode warning. Views arrive on `BlockComponentProps.node` / `document`, `EditorContext.document`, a `DecorationSource`'s `provide(document, …)`, the descriptor read hooks (`getContentRange`, `estimateHeight`, `reservedChrome.isCollapsed`, `reservedChrome.expandPatch`), and the command contexts.
...
Enough preamble. We are going to build a `:::note` container: a titled, editable box whose title is a real editable line and whose body holds ordinary Markdown blocks. Every import comes from the package, so this runs unchanged in a fresh SvelteKit app that installed the editor five minutes ago.

It reuses the `:::name` grammar rather than a hand-written opener. The grammar's tiers, dispatch, and losslessness belong to the [directives guide](directives.md); here we own the descriptor and the component side.
...
function registerNote(): void {
	...
	// falls through to the generic directive fallback. Guard each claim: a name is
	// registered once, and both of these are also claimed by the bundled admonitions
	// plugin, so an unguarded call throws the moment a consumer installs both.
	for (const name of [NOTE, 'tip']) {
		if (!isDirectiveRegistered('container', name)) {
			registerDirective('container', name, { kind: note, fromDirective: noteFromDirective });
		}
	}
	...
	registerBlockKind(note, {
		...
		// this, two adjacent notes give the user nowhere to type a paragraph between them.
		gapEdges: 'both',
		container: {
			...
			// trips a dev assertion the moment someone edits a note with a blank first line.
		...
		// cell or column is a compile error, and four more rules are checked at bootstrap.
		// See "The closure block" below for all of them.
		closure: {
			...
			clipboard: {
				mode: 'implemented',
				via: 'byte-slice copy; a slice touching the title re-emits the note around the collected body'
			},
...
`Record<ClosureColumn, …>` makes a missing column a compile error and the required field makes a missing block one. Four coherence rules also hold at bootstrap: a container must declare `roundTrip: implemented` (its `rebuildRaw` is the mechanism), a `not-mergeable` kind cannot declare `mergeBackspace: inherit-default` (it has no default merge to inherit), a cell claiming the focus-then-delete model must be backed by `blockFocus: 'whole-block'`, and a kind declaring `reservedChrome` cannot leave `clipboard: inherit-default` (the chrome bytes live in the container's own raw, so the default slice is wrong for it). Those four plus the nine columns are the whole contract; you need nothing outside this guide to fill a row.
...
Your component supplies only its own chrome; `createContainerBlock` hides the child-list state, the ancestor wiring, and the windowing. Pass `node`, `index`, and `path` as **thunks**, which is to say `getNode`/`getIndex`/`getPath`, each a live read re-evaluated per use. A captured value would snapshot stale state, and on this frozen surface the type no longer lets you pass one: a function-valued field is a live read, a plain-valued field is static config.
...
The chrome leaf is deliberately narrow: it is always present, single-line and unsplittable (paste flattens to inline), and a destructive range clears it rather than deleting it, so it stays the same kind through every edit. The contract guarantees the empty leaf's presence, not its look: an empty-state affordance (placeholder text over an untitled title, say) is yours to build with CSS on the leaf's block class.

**Declare `gapEdges` for every kind, and name the edges where your surface traps the caret.** A grid, a fence or an opaque embed leaves the boundary it shares with a neighbour unreachable: no caret can sit there, so no paragraph can be typed between two of them. `gapEdges: 'before' | 'after' | 'both'` on the kind opens the edges you name to a between-blocks caret, where typing or Enter mints a paragraph; `'none'` says your surface already hosts insertion at both edges. The field is required so that the no-gap answer is a decision you wrote down, never an omission. The bundled kinds set the precedent: an opaque container whose fences leave no textual way out (the callouts, details, the generic directive container) declares `'both'`; a container whose children can already escape at its boundary (a blockquote's unwrap, a list's exit) declares `'none'`.

Nested-editor interiors (a second editor's state serialized as a blob) are **rejected permanently**. They break byte-lossless round-trip, so there is no version of this that gets built.
...
- **`'plain'`**: the source is always the editable view, and every keystroke commits to the tree (prose-like undo batching). The spread's sync mirrors external rewrites (undo, a structural replace) into the source and gates `contenteditable` off the mode, so the always-mounted surface goes inert in reading mode; the factory owns the Chromium trailing-newline caret anchor and the caret restore.
...
Reactivity is **per tier, not universal**, and that is the honesty this section exists to state:
...
Answer `lines` **without** line endings, because the seam attaches the editing block's own, so a CRLF document stays CRLF. Answer the caret as a `path` (child indices inside the minted block, empty for the block itself) plus a `line` and `column` inside that node, never a byte offset: the seam picks the line ending after your claim, so only it can count bytes. The claim lands as one block replacement and one undo entry; one undo restores the typed line with the caret back at its end, and pressing Enter there completes again.

Two bounds worth knowing. Your lines are re-parsed by the ordinary parser, so a completer can only mint what a reload of those bytes would produce, which means registering the opener that recognizes them first. And a completer sees a line, never a position, so a grammar that is only legal at one place in the document is not a completion candidate.

## Inline kinds

Blocks are only half the story. An inline kind is minted with `declarePluginInlineKind`, recognized by hooking the scanner on a trigger character (`registerInlineSyntax`), and rendered as a live atomic widget (`registerInlineWidgetKind`). A widget renders through one of two paths, and the descriptor rejects declaring both:
...
- Pass reactive tree state by value across a module boundary. Hand it through a live read instead (a getter, or a `() =>` thunk as the frozen factory deps take).
...
Verifying what you built is [`plugin-testing.md`](plugin-testing.md): the round-trip checks, the test seam, and the conformance kits. Every export named above is cataloged in [`plugin-api.md`](plugin-api.md).
