...
| `getOptions`            | This editor instance's options for the plugin owning your kind, typed `unknown`: shorthand for `getEditor()?.options`, not a second source; the per-instance channel a factory argument can't reach ([the options recipe](#recipe-per-instance-options-and-the-factory-closure-trap))                                                               |
| `getEditor`             | This editor's `EditorContext` for the plugin owning your kind, undefined only in a bare harness; its `computeInlineContent` reads the syntax this editor draws. Hand `getEditor()?.computeInlineContent` to a helper whose reader defaults to the free `computeInlineContent`, as the bundled toc and footnotes do, and a bare harness still parses |
