...
| `computeInlineContent(node)`   | Parse a prose block's inline content the way this editor draws it, so syntax from a plugin its `plugins` prop left out comes back as plain text. Reach for it wherever you walk inline nodes     |
