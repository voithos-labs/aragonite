...
- **Never snapshot a live tier, and check the mode you handle.** A value captured at mount from a live getter is stale by construction — read it each time. And gate on `=== 'reading'` (the mode you render for), never a `'source'` check you invert, so a future preview rung degrades to your source rendering instead of disappearing.
