...
The plugin we're about to build is an homage to `curl parrot.live`: a line whose bytes are `%%parrot` renders as an animated ASCII party parrot, and any text after the marker becomes the parrot's caption.
