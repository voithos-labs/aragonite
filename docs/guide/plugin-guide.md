...
**The contract: every plugin tier can learn the editor's current presentation mode and render for it.** The editor is not permanently the marker-always source view — a consumer can put it in `reading` or `preview-block` mode today, and inline-granular preview follows — so a plugin that assumes source mode renders wrong the day its host flips the prop. `PresentationMode` is `'source' | 'reading' | 'preview-block' | 'preview-inline'`; every read below reports the **effective** mode (a not-yet-built rung reads as its closest built mode — `'preview-inline'` reads as `'preview-block'`), so you never render for a mode the editor is not actually in.
...
**The inline tier is not the block surface in miniature.** An inline kind gets recognition, rendering, atomic caret addressing at its edges, and an editing policy on its widget registration. The policy has four fields, all optional: `revealSource` (open the `$…$` source for editing on entry — inline math's model), `onSelectedKey` (a handler for keys while the widget is selected — image resize), and the two caret-edge fields the dispatch reads — `onEdge: 'select' | 'step-over'` (select the whole widget at an edge press, or step transparently over it) and `deleteGranularity: 'atomic' | 'select-then-delete'` (delete the whole widget on one press, or select-then-delete over two). The inline tier gets **no keymap, no minted commands, and no per-node metadata** — `InlineNode` has no metadata field, so unlike a block kind it stores nothing on the node. `deleteGranularity: 'atomic'` awaits its first shipped consumer (a decoded-entity widget); it is wired but not yet produced by a built-in kind.
