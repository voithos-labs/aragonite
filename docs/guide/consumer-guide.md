...
| `menuChange`             | `true` when an editor-owned menu (right-click, insert `+`, a typed-trigger list) opens and `false` when it closes; hide selection chrome meanwhile |
...
The tag source in `src/routes/test/plugins/tags/tag-marks-plugin.ts` is this recipe over a synchronous list, and `src/routes/test/plugins/inline-menu/doc-link-menu-plugin.ts` over a late one.
