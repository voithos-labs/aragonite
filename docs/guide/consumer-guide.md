...
4. **`opensAt` and `accepts` are your grammar.** A tag declines a mid-word `#` (so `C#` stays text) and ends on a space; a link accepts spaces and ends on `]`. `open(name)` skips `opensAt`: the gesture is the author's say-so. Your grammar only ever runs where the bytes are prose: a trigger typed inside an inline code span, a link's destination or title, an image, an autolink or raw HTML never opens, so a `#` in a URL fragment stays a fragment. A link's own text is prose, and a trigger there opens.
