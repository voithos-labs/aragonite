...
**Slash commands.** Type `/` at the start of a line or after a space and a list opens under the caret: the blocks the right-click "Insert block" rows offer (your plugins' blocks included), then Heading 1 to 3. Typing narrows it by label or keyword (`/td` finds the to-do list), Enter picks, and Escape closes it with what you typed left in place. It never opens in a table cell. On an empty line the block replaces the line; on a line with text the `/query` goes and the block lands below, and a heading row turns the line itself into a heading. Two rows take one word after a space: `/code js` opens a fence tagged `js`, and `/table 3x4` makes three columns and four rows. `Mod+/` types the `/` for you, and `runCommand('slashCommands.open', 'table')` opens the list already narrowed, which is how a toolbar button or a touch UI reaches it. `entries` adds rows of your own, each inserting Markdown (`insert`) or running a function (`run`, which receives the plugin's editor context and the typed word), and `exclude` hides built-in rows by id: `bullet`, `numbered`, `todo`, `quote`, `divider`, `code`, `table`, `h1`, `h2`, `h3`, and the ids your installed plugins add to `getInsertCatalogue()` (`math`, `note`). A row of your own with one of those ids takes the built-in row's place.
