...
2. **`bind:this` is the instance surface.** `getSource()`, `getSelection()`, `getEvents()`, `getSearch()`, `getRects()`, `getDecorations()`, `getDiagnostics()` read; `setSelection()`, `placeCaretAtPoint()`, `insertMarkdown()` and `runCommand()` write.
