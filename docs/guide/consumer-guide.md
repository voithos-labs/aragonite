...
| `blockDragHandles` | Opt into the pointer affordances: the block drag handle (on every block but a paragraph) and the table's row and column grips, revealed on hover and shown outright on touch (default off); keyboard reorder (Alt+Arrow) and the cell menu need no opt-in |
