...
- **`'preview-inline'`** — inline-granular preview (reveal per caret-adjacent inline span) is the last rung; until it ships it collapses to `'preview-block'`, the closest available behavior, with a dev-mode warning. Every mode read (below) reports the **effective** mode, so nothing ever renders for a mode the editor is not actually in.
