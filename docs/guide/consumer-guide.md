...
- **The prop is the enablement set.** Registration is shared; activation is per editor. An editor runs the hooks, resolves the kinds, answers the commands and their chords, and applies the paste transforms of exactly the plugins it lists, so leaving one out of an editor's array switches it off for that editor. Its syntax isn't in that editor's grammar either, so its blocks read as the plain Markdown they are, from the first parse on. Two things aren't scoped yet: a plugin's inline syntax, inline widgets, directive names and block completers still reach every editor, and a paste into one of its blocks is still answered by that kind's own paste surface. An editor mounted with no `plugins` prop is the exception: it activates everything installed in the process.
