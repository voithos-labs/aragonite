...
| `EditorContext`         | One editor's view for your plugin: `editorId`, the live read-only `document` and its `documentGeneration`, subscribe-only `events`, this editor's `options`, its `decorations`, `rects` and `inlineMenus` surfaces, the live `insertCatalogue`, `presentationMode` and `theme`, `insertMarkdown` (a promise that resolves once the insert has landed) and `runCommand`, which do what the instance's own methods do and answer false where those would, and `computeInlineContent`, an inline parse in this editor's syntax |
| `InsertMarkdownOptions` | `insertMarkdown`'s second argument: `placement: 'below'` puts the text into a new paragraph after the top-level block holding the caret, the way the right-click "Insert block" rows do, the paragraph and the text in one undo entry; the default inserts at the caret                                                                                                                                                                                                                                                     |
...
| `BlockKindRegistration`, `BlockKindDescriptor` | What `registerBlockKind` accepts, and the flat read-side shape lookups hand back; its optional `label` is what a screen reader and the block menu call the block                                      |
| `BlockKindAugmentation`                        | The patch `augmentBlockKind` merges; a partial `container` group merges into the existing one                                                                                                         |
| `RawWriteContext`                              | The third argument of a kind's `normalizeRawWrite`: the document's `lineEnding`, which a line your rule restores takes when the block has no ending of its own                                        |
...
| `lineStartsOuterBlock`, `OuterBlockScan` | "Does this line start a block at the outer level?" The shared end-of-extent test for a container opener scanning its own lines, and its inputs: whether a paragraph is open above, the grammar (required: pass the one your opener was given, so a syntax the editor left out starts nothing), and optionally the lines around it, for an opener that needs a later line to open _(pre-freeze / unstable)_ |
...
| `EditableLeaf`                                        | What the factory returns: the spreads, the focus surface your component re-exports, the mode and option reads, and `getEditor` for this editor's `EditorContext` |
...
| `InlineWidgetComponentProps`                              | A component widget's props: frozen `{ inline, source }`, plus live getters for the mode, the theme, the document, and the content version, `navigateTo` to jump to another block, optionally at an offset in it (aim at a leaf: a container path scrolls into view but seats no caret), and `computeInlineContent`, the editor's own inline parse                 |
...
| `BlockActionContext`                         | What `run` receives: the node, its path, `deleteBlock()`, `replaceRaw(raw)`, `transformPaste(text)` (the paste transforms of the plugins that editor lists), and `lineEnding`, the document's, which a line in `replaceRaw`'s bytes takes           |
...
| `BlockCommandOptions`                        | `registerBlockCommand`'s fourth argument: `movesBlock: true` marks a command that moves the focused block rather than the caret, so the keypress keeps the caret's column and marker side                                                           |
...
| `DecorationSourceHandle`                                                     | The handle back: `invalidate()` re-runs just your source, synchronously, except from an `edit` handler, where it waits for the commit to publish; `dispose()` removes it                       |
...
_(pre-freeze / unstable)_ A list the editor opens under the caret while the author types after a trigger: `#` for a tag, `[[` for a link to another document. You register a source per editor through `editor.inlineMenus` (your `onEditor` context); a host that owns the data reaches the same registry as `getInlineMenus()`. The editor owns the session: it notices the typed trigger, holds the arrow keys, Enter, Tab and Escape while a list is up, anchors the list to the typed range, and writes the pick over the trigger and the query as one undo entry. The caret never leaves the document. The recipe: [Recipe: a typed-trigger menu](consumer-guide.md#recipe-a-typed-trigger-menu).

| Export                   | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `InlineMenuRegistry`     | The `editor.inlineMenus` surface: `addSource` registers a source and returns its handle, `open(name, { query })` types a source's trigger, then the query if given, at the caret and opens its list over it (what a shortcut or a toolbar button calls), `close()` dismisses, and `isOpen` is true while a list is on screen                                                                                                                                                                                                                          |
| `InlineMenuSource`       | A named source: its `trigger` (one line, never empty), an `items(query)` that returns the list or a promise of it, and optionally `opensAt` (decline a trigger by position), `accepts` (end the session on a query that is no longer yours), `onCommit` (after the bytes land, with the range the pick replaced, so the caret is at `start + insert.length`; if it returns a promise, every write that lands while it is pending, up to the author's next input, is part of the pick's undo entry), and a `row` component that paints a row's content |
| `InlineMenuOpenOptions`  | `open`'s second argument: a `query` typed after the trigger, so the list opens already narrowed                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `InlineMenuSourceHandle` | The handle back: `dispose()` removes the source and closes its list                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `InlineMenuItem`         | One row: an `id` unique within the list, a `label`, an optional dim `detail`, an optional `icon` (a menu glyph name the default row draws), and `insert`, the bytes that replace the trigger and the query. Where two rows share an id the first is kept and the rest are dropped and reported on the `error` event. One line of inline bytes, empty allowed; a line break is refused and reported, and a block belongs in `onCommit`                                                                                                                 |
...
| Export                 | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POINTER_GESTURE_ATTR` | Put it on the element whose drags are yours. A press inside it is left alone by drag-to-select, and by the double and triple click too, as long as the element sits outside any `contenteditable` (an island inside your own editable text is the inline widget's own rule instead, where a third click selects the block unless the editor already holds that island selected whole). The editor reads it at press time, so declare it always or only while your gesture is armed |
...
| `parseContainerBody`, `ContainerBodyWrap` | Parse a body that sits between your container's own fence lines, keeping the separating blank lines out of the children; the wrap says which fence lines exist, and matches your kind's `bodyWrap`. The options take the grammar, which is required: pass the one your opener was given, so the body reads with the syntaxes the editor has on |
...
| `trailingLineEnding`                      | The ending a line rebuilt from a block's bytes takes: the block's own, else the fallback you pass (a write rule's `write.lineEnding`, a context action's `ctx.lineEnding`, or `documentLineEnding` of a document you hold)                                                                                                                     |
...
| `LineEnding`                              | `'\n'` or `'\r\n'`                                                                                                                                                                                                                                                                                                                             |
| `displayLines`                            | A block's display as lines, each line's text apart from its ending, so a `\r` never reaches a line match                                                                                                                                                                                                                                       |
