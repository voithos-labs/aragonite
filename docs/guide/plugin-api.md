...
| `InlineMenuSource`       | A named source: its `trigger` (one line, never empty), an `items(query)` that returns the list or a promise of it, and optionally `opensAt` (decline a trigger by position), `accepts` (end the session on a query that is no longer yours), `onCommit`, and a `row` component that paints a row's content |
