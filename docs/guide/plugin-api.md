...
| Export                   | Role                                                                                                                                                                                                                                                                               |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `InlineMenuRegistry`     | The `editor.inlineMenus` surface: `addSource` registers a source and returns its handle, `open(name)` types a source's trigger at the caret and opens its list (the shortcut and toolbar-button door), `close()` dismisses, and `isOpen` is true while a list is on screen         |
| `InlineMenuSource`       | A named source: its `trigger`, an `items(query)` that returns the list or a promise of it, and optionally `opensAt` (decline a trigger by position), `accepts` (end the session on a query that is no longer yours), `onCommit`, and a `row` component that paints a row's content |
| `InlineMenuSourceHandle` | The handle back: `dispose()` removes the source and closes its list                                                                                                                                                                                                                |
| `InlineMenuItem`         | One row: an `id`, a `label`, an optional dim `detail`, and `insert`, the bytes that replace the trigger and the query                                                                                                                                                              |
