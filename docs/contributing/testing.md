...
A shape a guard's own oracle cannot survive belongs in that guard's fixed corpus rather than in a shared lane. Asterisk delimiter nesting is the standing example: it rebinds under any neighbouring byte, which the typing-seat net reads as its own failure, so it lives in the G2.14 display corpus and not in `arbInlineSource`.
