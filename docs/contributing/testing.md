...
Two layers, both living inside `src/lib/` next to the code they test. There is no separate test
tree at the repo root, and that is on purpose: the whole editor module is self-contained
...
- [The conformance differ](#the-conformance-differ): our inline parser diffed against
  commonmark.js.
- [Property suites and fresh seeds](#property-suites-and-fresh-seeds): why the random tests are
  not random, and the switch that makes them random again.
...
Beside the two layers sits `src/lib/testing/`, which is not a test layer but **shipped code**:
the published `@voithos-labs/aragonite/testing` module, holding the plugin-platform reset and the
kind, container and inline conformance kits a plugin author runs inside their own test suite. It
is a seam in the honest sense (a boundary where responsibility passes from one piece of code to
...
(the kits driven exactly as an author would drive them).

The three commands you will type most:
...
invariant that matters most, here and everywhere else: `serialize(parse(source)) === source` for
all valid GFM.
...
their own script run in the full `test:editor` suite. A new area is cheap to create when a
directory earns one: it is one `vitest run src/lib/test/<dir>` line in `package.json`, nothing
else.
...
only what it asserts on (`editorMountContext({ blockEdit, doc: { doc: () => parsed } })`) and
takes sensible stubs for the rest. When the editor grows a newly required context, that costs
one harness edit instead of a fix across every block-mount test.

### A dev warning fails its test

The verdict first: every `devWarn` fire reaches a structured sink the unit setup registers, and
a fire no test claims **fails** that test. Not warns. Fails. The sink takes reporting over, so
under the unit runner the console line never happens and a `console.warn` spy sees nothing. The
same per-test hook resets `editorEnv`, so a suite that configures the environment configures it
per test; a `beforeAll` override is gone by the second case.
...
The machinery, for when you are inside it:
...
- Two per-file aggregates close what a per-test verdict cannot see: a tag declared through
...
- There is no exemption. A file that `vi.mock`s `$lib/dev-warn` deletes the emitter outright,
...
`@playwright/test` directly.** This is not a style preference, and I will be tedious about it in
...
spec that _caused_ it rather than passing silently and turning up a release later. A spec that
deliberately trips one names its tags:
`test.use({ expectInvariants: ['late-opener-registration'] })` for an invariant fire,
`test.use({ expectWarns: ['tree-ops'] })` for a plain dev warning. Both run in both directions:
a named tag that stops firing fails too.
...
- **`editor.bridge`** is the _state_ accessor: `getSource` / `getBlockCount` / `getBlockKind`,
  ...
  of `waitForTimeout` whenever you are waiting on document state.
...
`npm run test:e2e:isolated` runs the whole suite that way; for anything narrower, compose the
variable through the launcher:
`node scripts/run-with-env.mjs E2E_ISOLATED=1 -- playwright test --project=e2e-blocks`.
...
flake surface for a class of bug that does not appear between releases.
`npm run test:e2e:webkit` sets `WEBKIT=1`, and that variable is what makes the `e2e-webkit`
project exist at all, so the run cannot half-happen inside the default suite. It collects a
...
helpers that branch on the engine. It fails rather than reports, which it can afford because it
...
chord cannot see; so the WebKit side seeds, pastes and reads through a dispatched clipboard
event carrying a `DataTransfer`, which is where the editor's own handlers already read and
write. WebKit exposes no CDP session, so the IME driver hand-fires the composition sequence: the
one exemption G4.49 grants, which no spec may copy.

What the run proves: editor behavior survives a second engine, and the commit path survives a
WebKit-shaped composition without double-applying at `compositionend`. What it does not: event
...
**The caveat, wherever this run is described.** WebKit-on-Windows through Playwright is not
Safari and not a WKWebView. It is the closest available proxy, so a green run is weaker evidence
than its pass count suggests, and #37 stays open until something runs on Apple hardware. The
browser build is pinned by the `@playwright/test` version in `package.json`: a Playwright
upgrade that turns this run red is signal, not noise.

### Requirements pair one-to-one with specs

Every spec under `src/lib/e2e/tests/` pairs with a requirement file under
`src/lib/e2e/requirements/`, a plain-English list of scenarios, written _before_ the spec.
Before. That word is doing real work. The filesystem is the authoritative list of what's
covered: a spec with no requirement file, or a requirement file with no spec, means one of the
two is out of lockstep. Fix it, don't work around it.
...
Note the import path: `../fixtures`, not `@playwright/test`. That is the invariant watcher, and
it is the one line in this file most worth not copying wrong.
...
much fun to diagnose as it sounds. Never fire unsettled `keyboard.type` in a tight loop.
...
and gets an inline comment when used. The raw rebuild itself is synchronous; you are waiting on
...
reparse cannot. `getDomBlockCount()` counts _mounted_ top-level blocks, which under virtual
...
Always follow the structural op with navigation through a container. See the
...
correct regardless of where focus landed, so it cannot detect a focus bug at all. To verify
...
one.** An `<img>` that has not decoded lays out 0x0, and `.md-image-widget` shrink-wraps it, so
a rect read too early is degenerate. Compute a point from that rect and the click lands _inside_
...
`page.evaluate(() => el.getBoundingClientRect())` do not. So a raw-`evaluate` or
...
`Input.insertText` to commit; see `tests/ime-composition.spec.ts`. Mid-composition there is no
...
divergence always means the _inline_ parsers disagree, never that the block layers trimmed
differently.
...
bytes still tile. That is the gap a byte-conservation or offset-tiling property cannot see. It
allows only the divergence classes the baseline documents as deliberate.

## Property suites and fresh seeds

_Why not draw a fresh random seed every run?_ Because a gate that fails differently each time is
a gate people learn to ignore. Property/fuzz suites (`fc.assert` over the shared arbitraries)
run **fixed-seed** so the commit gate is deterministic: a regression surfaces the same way every
run rather than as a flake. The cost is no new-input discovery over time, since one seed
explores one set of inputs.
...
threads through the `freshOrFixedSeed` helper) and runs just the property-bearing suites. Run it
when touching the inline parser, the CST, or the arbitraries, or just periodically, to hunt
inputs the fixed seed never reaches. It is never part of the gate; reachability self-tests keep
their fixed seeds so they cannot flake.

**What the generators must draw.** A generator that cannot produce a shape proves nothing about
...
is not well-formed UTF-16. The inline source generator carries a second floor over construct
...
A shape a guard's own oracle cannot survive belongs in that guard's fixed corpus rather than in
a shared generator; but shedding the shape is the second answer, and teaching the oracle to
...
**Reproducing a fresh find.** Fresh mode prints its seed (`[property:fresh] seed <N> …`) before
the run, and fast-check echoes the failing seed and shrunk counterexample in any failure. To
replay, pin that seed as the site's fixed default; the durable fix is to add the counterexample
as a committed regression case, which guards the class without the mode.
...
issue turns the pin red and the exclusion goes with it. An exclusion without an issue number is
not allowed, no matter how obvious it looks at the time.

One oracle stands outside that sort: UTF-16 well-formedness. A gesture that writes a lone
surrogate its input did not hold fails the sweep whether or not the twin writes one too, because
no rebinding excuses bytes that no UTF-8 boundary round-trips and no inverse gesture restores.
It is why the two gestures whose offset a caller computes rather than the engine reporting it,
...
full minor version without it looking, which is not a stretch I would like to repeat.
...
bold/italic, list alignment, a resized image's width, the right block kind) is not easily
