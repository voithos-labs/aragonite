...
Call `registerBlockKind(kind, registration)` in `schema/block-kind-descriptor.ts`. Core fields: merge role, `editable`, `supportsInline`. `BlockKindRegistration` documents every optional hook; the ones you're most likely to want:
