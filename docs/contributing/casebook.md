...
child straight off the DOM, turns a DOM position back into raw with `rawOffsetAt`, and writes a
caret with `placeCaretAtRaw`. Offset math done anywhere else agrees with it right up until it
...
fails any native selection write that isn't `placeCaretAtRaw` or one of the few declared writers
that select whole nodes.
