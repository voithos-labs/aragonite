...
| Arrow at a block boundary   | `src/lib/selection/shared-keydown.ts` :: `handleSharedKeydown`                                                                                                                 | Vertical exits read geometry (the first/last visual line); horizontal ones read the block's content bounds, which a no-reveal mode moves in from raw 0 | `docs/design/editor.md` § 8                            |
...
| Caret edge affinity         | Captured via `note` on `src/lib/cursor/edge-affinity.ts` :: `createEdgeAffinityState`, from `src/lib/selection/shared-keydown.ts` :: `handleSharedKeydown`                     | One arrival door and one editor-scoped instance; a write seam reads `get()` and keeps its own default when it answers null                             | `docs/design/editor.md` § 8                            |
