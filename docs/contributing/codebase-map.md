...
| Caret edge affinity         | Captured via `note` on `src/lib/cursor/edge-affinity.ts` :: `createEdgeAffinityState`, from `src/lib/selection/shared-keydown.ts` :: `handleSharedKeydown`                     | One arrival door and one editor-scoped instance; a write seam reads `get()` and keeps its own default when it answers null                                          | `src/lib/cursor/edge-affinity.ts` header               |
