...
1. **Ship gate, on the release tree.** `npm test`, `npm run check`, `npm run lint`, then `npm run test:e2e:webkit` on its own (the second-engine lane sits outside `npm test` and fails rather than reports; `docs/contributing/testing.md` § The WebKit lane). Alone means a quiet tree too, not just no concurrent run: a save into `src/` while it runs reloads the SSR side, the block components re-register, and the server-warn reporter reds the run for a reason the product never had. Then `npm run perf:check` on the calibration machine (it builds and previews the app first, so budget the extra minutes; any other host reads red by design, and CI's scaled perf job is the cross-check). Capture output to files, check exit codes, never pipe.
