...
- **commonmark.js conformance harness** (`src/lib/test/conformance/`). Both parsers normalize to one minimal inline shape — unmapped constructs throw, nothing is silently absorbed; the one deliberate reconciliation (empty-vs-absent link title, which commonmark's AST cannot distinguish) is recorded in the baseline's audit array — and diff over three corpus strata (378 inline-only spec examples as escaped-JSON fixtures, brute-force enumeration, seeded random). A like-for-like guard (commonmark sourcepos full-input-span) skips inputs the reference's _block_ layer trimmed or consumed, with per-reason skip accounting, so every divergence is inline-semantic. Two tiers: a deterministic slice rides `npm test` against a 71-entry committed baseline that fails closed in **both** directions (new divergence fails; stale entry fails until removed), and `npm run conformance:full` is the env-gated sweep meter — 182,160 inputs compared; 51 baseline-classed + 1,822 unclassified divergent inputs are the scanner-rework work list. The reference is pinned exact at commonmark 0.31.2; bumping it is a deliberate re-bless. The first full-scale sweep immediately caught an arg-stack overflow in the corpus enumerator. Miss-analysis: enumeration counts were pinned only at tiny lengths; the full-scale parameters had never executed until the meter ran.
