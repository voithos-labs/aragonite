...
- **Marks are rows.** Bold/italic/strike/code nesting order, marker bytes, and command ids are
  policy-table rows a plugin construct can declare, replacing the hand-written mark lists; a
  plugin mark row claiming a built-in command id is refused at registration.
