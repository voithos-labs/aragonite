...
### 0.9.42: the audit's fixes — merges that refuse to corrupt, live gestures that all verify

- **A merge can no longer eat a line or diverge from its own reload.** Delete at the end of a
  heading before a multi-line paragraph silently dropped a line; Backspace the other way kept
  the bytes but reopened as a different document. Both sinks now refuse a join whose bytes
  reparse to several blocks and move focus across the boundary instead.
- **Every destructive live gesture crosses the verification seam.** Word-delete, line-delete,
  drag-delete and spellcheck replacements went native and could strand literal delimiters on
  screen; they now classify by target range and route through the join cleanup. A live
  selection toggle over boundary whitespace wrote asterisks that painted (`**hello **world`);
  it now moves the space outside the wrap or declines. A range delete that emptied a link left
  `[](url)` invisible in the file; the join now unwraps what the cut emptied.
- **Global keybinding overrides work at every focus state.** A host's rebind or disable
  resolved only at text surfaces and was dead at the editor root, the gap caret and
  whole-block states; one override-aware seam now serves all of them. A disabled chord's
  contract is stated: released for the host outside the editor, still consumed inside it.
- **`editLink` declines under a cross-block range** on both the chord and `runCommand` paths
  (it could open the create card over bytes the user never selected), and Mod+K joins the
  cross-block swallow.
- **No gesture can split a surrogate pair.** Offsets snap to scalar boundaries at the
  selection funnel and the cut seams, so `setSelection` plus delete can no longer destroy half
  an emoji irrecoverably.
- **Reported fixes.** Deleting beside a document's folded trailing blank line converges on
  reload and keeps block identity aligned; a commit into a never-mounted container keeps its
  child ids; pasted nested GitHub alerts convert in one pass.

### 0.9.41: the command door, typed math, and the live-mode hardening

- **`editor.runCommand(id)`** — semantic command invocation on the instance, what a selection
  toolbar calls instead of synthesizing chords. `TOOLBAR_COMMANDS` on the barrel names the five
  toolbar ids; the demo's selection toolbar is the reference. The cross-block format-toggle
  decline moved to the one dispatch seam both chords and the door cross, so a rebound chord or
  an id dispatch can no longer reach a single-block arm under a cross-block range.
- **A table typed inside a list item now completes** — the Enter-completion consult moved to
  the composed bundle seam, so every container's split (the list item included) crosses it.
- **Block math typed into existence.** With the latex plugin installed, `$$` + Enter mints the
  fence pair with the caret in the body; without it, the line splits like any other paragraph.
  Plugins can register their own completers: `registerBlockCompleter` on `aragonite/plugin`,
  and a completer whose mint would paint nothing is declined.
- **Live mode drops no painted byte.** A construct standing over content-empty chrome
  (`**[](u)**`) showed its delimiters while the cut seams read them as unseen: a range delete
  could drop them, a split could relocate them. Every live rewrite seam now declines a side
  whose chrome paints, and a destructive press whose candidate re-reads as a different block
  (a minted `~~~~` fence swallowing the document below) takes nothing instead.
- **Blank lines settle at one funnel.** The separator-settle doors moved into the commit
  ceremony, closing the class where filling, emptying, pasting over, or deleting a blank block
  left bytes that reparse to a different shape on reload — including the list break-out paste
  and empty-replacement arms that had never settled at all, and a callout emptied at its body
  tail now returns the line it borrowed from the closing fence when refilled.
- **Marks are rows.** Bold/italic/strike/code nesting order, marker bytes, and command ids are
  policy-table rows, replacing the hand-written mark lists at every toggle surface; a mark row
  claiming a command id that already has a built-in meaning is refused at registration.
- **Reported fixes.** A caret walk into `![a](a)` no longer mislocates the reveal
  (self-similar source); the reference-label chrome classification agreed across the walk and
  the painter (#141); undo entries restore ref slots correctly after container commits
  (a Svelte teardown pinned pre-flush reads and stranded cleared refs); decorations re-derive
  against the document they were computed from, so a keystroke no longer flags its own islands.
...
- **A space after a typed `>` completes the marker instead of doubling it.** A typed `>` mints
  the quote at once, so the following space used to land in the empty first line and indent the
  quote's content by a byte forever (`>  a`). The space is now consumed as the marker's own
  completion — the canonical `> ` re-emits the moment content arrives — matching the list, whose
  `- ` only ever forms whole. Every bare space at an empty quote child's content start is
  consumed the same way.

- **Typing at a focused thematic break or diagram starts a paragraph below.** A printable pressed
  while a whole-block kind held focus was silently dropped. It now mints a paragraph below
  carrying the character with the caret after it, the gap caret's convention, as one undo entry;
  chords keep their existing routes, and reading mode stays inert.
...
- **The demo app is now the pitch.** The `/` showcase document reads as a tour — every block
  ...
  the "under the hood" view. A new `/changelog` route renders aragonite's own changelog — a real
  document of thousands of lines, under virtual rendering with find over it — behind a collapsed
  Versions outline, defaulting to reading mode with one toggle flipping the same bytes to styled
  source: the single-render-path claim, live.
