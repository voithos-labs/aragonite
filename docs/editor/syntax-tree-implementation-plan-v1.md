...
describe("splitLines", () => {
  ...
  it("handles single line no newline", () => {
    ...
    expect(lines).toEqual([
      { raw: "hello", text: "hello", lineEnding: "", start: 0, end: 5 },
    ]);
  });
});
```
...
describe("round-trip: leaf blocks", () => {
  const cases: { name: string; source: string }[] = [
    ...
    { name: "mixed content", source: "# Title\n\nParagraph text.\n\n```\ncode\n```\n\n---\n\nMore text.\n" },
  ];
...
function startsNewBlock(text: string): boolean {
  ...
  return Boolean(
    matchFenceOpen(text) || matchHeading(text),
  );
...
function parseBlockquote(
  ...
  const quoteDepth = lines[startIndex].text.match(/^ {0,3}(>[ \t]?)+/)?.[0]
    .split("")
    .filter((c) => c === ">").length ?? 1;
...
function parseList(
  ...
  while (i < endIndex) {
    ...
    const innerParagraph = innerText.length > 0
      ? [new Paragraph("", innerText + itemLine.lineEnding)]
      : [];
