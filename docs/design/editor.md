...
- **Paint.** Every `BlockHost` mounts a `MatchOverlay` beside its `SelectionOverlay`. Matches are grouped by owning path once per scan, so an overlay reads only its own bucket instead of scanning the document on every scroll. Windowing follows for free: an unmounted block simply doesn't paint.
...
- **Cost when idle: zero.** The rescan is an `edit`-event subscriber that runs only while the bar is open, and it is deferred off the commit path, never a synchronous per-keystroke scan.
