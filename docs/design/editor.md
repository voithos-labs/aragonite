...
DOM focus does not sit on the declared surface. The kind declares which element **stands for** the block, and the editor mounts a hidden editing host in the block's box and focuses that instead — AltGr productions and IME composition reach an editing host or nowhere, and a `tabindex=0` div is not one. It is the gap caret's proxy technique (§ 10) at a second site, shared as one factory rather than twinned; the declared surface keeps focus only when it is itself editable, owning its caret already. Consequences worth knowing: a focus assertion is containment, not identity, and a plugin box needs `position: relative` for the host to resolve against the block.
