...
2. **Point resolution.** Turn a viewport point into a landing first: the `caretTargetAtPoint` descriptor hook answers for coordinate-addressed kinds (a table names a cell), with the drag hit test (`foreignDragHitTest`) as its decline-happy sibling.
...
registerBlockKind('blockquote', {
	...
	container: {
		...
		unwrapRole: { firstChildBackspace: 'lift-first-child', middleChildBackspace: 'default-merge', quoteShaped: true },
