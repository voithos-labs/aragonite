...
**Merge.** Concatenate two adjacent nodes' `raw` and re-parse to determine the merged kind, where the concatenation reads as **one** block. A join whose bytes reparse plural is refused at the write, and the caret moves across the boundary instead. The survivor keeps its ID. The second block's text lands at the first one's content end, so a structural suffix (a setext underline) stays under the joined text; a range delete that starts in such a block keeps it the same way, and one that ends in such a block drops that block's suffix along with the block.
