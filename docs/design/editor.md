...
The editor exposes an observer surface via `getEvents()`: six channels, where `on(name, cb)` returns a disposer. Events fire synchronously from their emission sites, and handlers must not mutate the document (reentrant edits aren't supported).
...
- **`menuChange`.** `true` when an editor-owned menu (the right-click menu and its flyouts, an inline menu's list) opens and `false` when it closes, so a host's own controls over the selection can step aside rather than stack.
