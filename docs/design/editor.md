...
- **Enter-completion registry** — the opener registry's sibling, for kinds whose grammar spans adjacent lines and so cannot be typed into existence. A kind registers a completer that reads one typed line and answers the canonical lines completing it plus the caret's landing; the split command consults it at one arm (§ 8). Internal, not part of the plugin surface.
