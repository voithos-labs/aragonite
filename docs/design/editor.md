...
**Reorder.** Move a node among its siblings; IDs don't change, and both edges of the moved window are settled. Two gestures, one operation: keyboard (Alt+↑/↓ on the focused block, with a screen-reader announcement) and a pointer drag from the block's handle, revealed on hover and shown outright where nothing hovers (an insertion line marks the drop, one commit on release, autoscroll for off-screen targets). The handle is opt-in (`blockDragHandles`, default off), skips paragraphs (the page's background), centres its grip on the block's first line, and that switch covers the table's row and column grips too, the other pointer-only reorder affordance. Keyboard reorder is always available.
