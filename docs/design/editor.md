...
Re-reading every child costs the container's whole width, which is what a keystroke inside a 25,000-item list was paying. So a container also carries **child spans**: where each child's bytes sit inside its own `raw`. When the caller names the one child whose raw moved — the typing door does, since it is the only one that knows — the rebuild rewrites that child's region and shifts the spans behind it, reading one child instead of all of them. Every other caller re-derives the whole raw, which reseeds the spans, so the incremental path can never drift far. Three rules keep it honest: the spans are bookkeeping and never bytes (nothing serializes or renders them); a children-shape change drops them at the seams that hold the container node, and elsewhere their count no longer matching the children is itself the signal; and a rebuild that finds the named region no longer holding the bytes it expects re-derives instead of writing into it, which turns any stale span into a slower rebuild rather than a corruption. A plugin rebuilder that ignores the hint is correct, just not incremental.
