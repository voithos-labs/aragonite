...
**Augmentation is distinct from registration.** `augmentBlockKind` merges fields into an _existing_ registration; the top-level wire-up uses it to patch in behavior that can't live in the schema layer without a downstream import. Augmenting an unregistered kind throws. Augmentation is deliberate and idempotent-by-intent; registration is once. And there's no unregister and no replace: runtime plugin loading and unloading with sandboxing is Plugin System II, explicitly outside this contract.

**A global opener means global syntax recognition, resolved through a per-instance view.** A plugin that registers a block opener teaches the _parser_ to recognize that syntax process-wide, so by default a second editor in the process parses plugin blocks even if its consumer never passed the plugin. Definitions stay global; each instance _resolves_ them through a `RegistryView` (`schema/registry-view.ts`) whose default reads every definition verbatim, so behavior is unchanged. That view is the designed-ahead infrastructure for the per-instance **enablement** policy layer this contract has always placed above the definitions. `parse` gained an additive `parse(source, { grammar })` slot that threads an instance's grammar view (the default is the global grammar), so a filtered instance can skip a disabled kind's opener. The enablement _predicate_ has no public prop yet; it firms up with limestone. The view and its semantics are frozen-safe because they're additive over the global definitions: built-ins are never disableable, a disabled kind's component resolves to the raw-editable fallback, and the descriptor is never filtered.
...
_Why global at all, given the per-instance `plugins` prop?_ Kind _definitions_ are global because, like custom elements, a kind can't be defined differently for two editors in one process. The `plugins` prop is a registration _trigger and declaration of intent_, so passing the same plugin to two editors registers once. Per-instance _enablement_ (editor A renders callouts, editor B doesn't) is the policy layer _over_ the global definitions, not per-instance registration, and its structural piece is built (the `RegistryView`, 0.9.27). What's deferred is only the public prop that sources the enablement predicate, which firms up with limestone rather than being guessed at now.
...
A container may contribute an ambient prefix (the read-only marker a container lends its first prose child, the way a list lends `- `) through the factory's optional `getAmbientPrefix` dep, a live getter forwarded to the nested list as `ambientPrefixForFirst`, riding the same offset walk as the listItem model; the footnote definition's `[^label]: ` marker is the validating consumer. A prefix may also mark ranges of itself interactive, which is how a task list's checkbox toggles and how that footnote marker takes the click back to its reference: the range gets its own span, class and click handler, and the caret still clamps out of the whole prefix.
...
- **`EditEvent`**: `{ op, path, detail, timestamp }`, where `op` is the `OperationKind` vocabulary derived from `OperationDetailMap`. Emitted from the commit ceremony (the fixed steps a commit always runs) for structural ops, and from the keystroke-debounce flush as `op: 'input'`.
...
A _general_ editable leaf shipped pre-1.0 as `createEditableLeaf`, beside the container factory; the chrome leaf stays narrower on purpose. **Rejected permanently:** nested-editor interiors, a second editor state serialized as a blob. A blob can't round-trip through the CST, and that ends the conversation.
...
- **Path-addressed caret placement.** Shipped, as the growth rule predicted: a sibling member on the same surface (`rects.navigateTo(path, offset?)`: reveal, scroll, land the caret), never a second navigation object. What resolved the deferral wasn't a new design but the discovery that the write already had one home: a navigation runs the shared restore road, so cross-block normalization, the deep-versus-shallow cursor contract, and undo-visible focus are answered by the code `setSelection` and undo already route through, rather than by a second copy of the rules. The `offset` argument followed once it had a consumer: a footnote definition's marker jumps back to land just past its citation, so `navigateTo` now takes an offset into the target and defaults to the block's start, which is what a navigation usually means.
- **Inline-widget editing-policy re-add.** The two trimmed `InlineWidgetEditingPolicy` fields have been re-added, `deleteGranularity: 'atomic' | 'select-then-delete'` and `onEdge: 'select' | 'step-over'`, additively, when the caret-edge dispatch consolidated the widget, island, and ambient handling into one declarative dispatch (`components/blocks/text/edge-policy-dispatch.ts`). `onEdge` distinguishes selecting the construct whole from stepping transparently over it; `deleteGranularity` distinguishes the two-press select-then-delete (image, `<br>`) from a one-press whole delete. Both now have a consumer: the built-in decoded-entity widget (`&copy;` becomes ©) ships `{ deleteGranularity: 'atomic', onEdge: 'step-over' }`, defining atomic delete (a caret-adjacent Backspace removes the reference whole, a plain arrow steps the caret across the glyph), and the dispatch reads both off the widget registration.
...
- **Unified command registry + palette.** Built-in block commands still dispatch through `component.runCommand` rather than the `(kind, name)` registry the command surface publishes, so dispatch has two homes and anything enumerating commands can see only one of them. Retiring the first onto the second is internal work: the registry shape a plugin binds today is unchanged, because a command is already a function of a context rather than a method on a view.
- **Per-instance enablement prop.** The instance-resolution piece ships now (the `RegistryView` over the global definitions): component, descriptor, and grammar resolve per instance, and a filtered view degrades a disabled plugin kind to the raw-editable fallback and skips its opener. What's deferred is the **public prop** that sources the enablement predicate: it arrives additively (a new optional `Editor` prop, no restructure of the `plugins` prop) and its shape firms up with limestone rather than being guessed at. Today the predicate rides a harness-only route for the proof.
...
- **Per-hook post-1.0 surfaces**, meaning selection coordinate-addressing, and the component registry replacing `BlockHost` dispatch. Both additive over this foundation. (The component-portal widget surface already shipped additively, pre-1.0.)
