...
- **Inline-widget editing-policy re-add.** The two trimmed `InlineWidgetEditingPolicy` fields have re-added — `deleteGranularity: 'atomic' | 'select-then-delete'` and `onEdge: 'select' | 'step-over'` — additively, when the caret-edge dispatch consolidated the widget/island/ambient seams into one declarative funnel (`components/blocks/text/edge-policy-dispatch.ts`). `onEdge` distinguishes selecting the construct whole from stepping transparently over it; `deleteGranularity` distinguishes the two-press select-then-delete (image, `<br>`) from a one-press whole delete. `atomic` is honored by the dispatch and pinned by a unit test, but has no shipped kind — it still awaits the inline-entity consumer (`&copy;`) that defines it, so the value is wired ahead of its first producer rather than frozen inert.
