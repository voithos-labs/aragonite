...
- **Inline prefix-recognizer tier.** A precedence-override variant of `registerInlineSyntax` for a recognizer that must outrank a built-in trigger on a longer prefix — made concrete by the footnotes probe: a GFM `[^label]` reference needs to win `[` on the two-character prefix `[^`, and the tier must define unterminated-construct behavior (a `[^` that never closes falls back to the built-in's reading — never a hang, never a byte change). Additive over the base hook: the base signature and its reserved-trigger throw are untouched; the tier arrives as a new registration form. Footnotes is the validating consumer when it is built.
