...
1. **◐ Clipboard.** Both chrome directions round-trip the container: an end landing mid-chrome yields a chrome-only container, a start landing mid-chrome reopens the container around the collected body and closes it where the walk leaves the subtree. What remains partial is depth: an endpoint inside a _nested_ container's body is skipped as an endpoint ancestor, so that container's own wrapper is lost while the outer one survives (ledgered, `docs/issues.md`; folded into the post-1.0 clipboard generalization).
