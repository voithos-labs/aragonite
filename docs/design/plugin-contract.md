...
| Presentation-mode contract (`PresentationMode` + per-tier mode reads)    | breaking-if-deferred | **Yes — mode contract + reading rung shipped pre-1.0; preview rungs render as source** | A plugin authored against a marker-always editor renders wrong the day preview ships — not an API break but an ecosystem-stranding one; the union ships whole so later rungs activate without any API change    |
...
`PresentationMode` (`'source' | 'reading' | 'preview-block' | 'preview-inline'`) is the vocabulary; every read reports the **effective** mode (a not-yet-built preview rung collapses to `'source'`), so nothing renders for a mode the editor is not in. One vocabulary, four doors:
...
2. an internal block-facing context getter that joins the prose render key, so a mode flip re-renders every mounted block;
