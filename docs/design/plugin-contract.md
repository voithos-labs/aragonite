...
| Presentation-mode contract (`PresentationMode` + per-tier mode reads)    | breaking-if-deferred | **Yes — all four rungs shipped pre-1.0 (mode contract, reading, block-granular, inline-granular preview)** | A plugin authored against a marker-always editor renders wrong the day preview ships — not an API break but an ecosystem-stranding one; the union shipped whole and the last rung activated with zero API change                                         |
...
1. **◐ Clipboard.** Both chrome directions round-trip the container: an end landing mid-chrome yields a chrome-only container, a start landing mid-chrome reopens the container around the collected body and closes it where the walk leaves the subtree. What remains partial is the generic case the chrome pair sits inside: an endpoint landing in a container's body is skipped as an endpoint ancestor, so unless one of the four recovery seams applies, that container's wrapper is lost (ledgered, `docs/issues.md`; folded into the post-1.0 clipboard generalization).
