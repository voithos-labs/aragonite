...
The view is frozen-safe because it's additive over the global definitions: built-ins are never disableable, and the descriptor is never filtered. `parse(source, { grammar })` threads the instance's view, so a reparse skips an unlisted kind's opener; the **initial** parse still reads the global grammar, so an unlisted plugin's syntax parses to its kind and then degrades on render. Two leaks are open on the ledger: inline syntax, inline widget kinds, directives, and block completers still reach every editor (#266), and `reservedChords()` still enumerates every registered kind's keymap, so it over-reports an unlisted plugin's per-kind chords: the answer is wrong, not the dispatch, since that kind resolves no surface to press one on (#288). The harness-only `__registryEnablement` prop stays, because the filtered-view proof wants a predicate it can vary without varying the install.
