...
| Presentation-mode contract (`PresentationMode` + per-tier mode reads)    | breaking-if-deferred | **Yes — mode contract + reading + block-granular preview shipped pre-1.0; inline-granular renders as preview-block** | A plugin authored against a marker-always editor renders wrong the day preview ships — not an API break but an ecosystem-stranding one; the union ships whole so the last rung activates without any API change |
...
`PresentationMode` (`'source' | 'reading' | 'preview-block' | 'preview-inline'`) is the vocabulary; every read reports the **effective** mode (`preview-inline` collapses to `preview-block`, the closest built rung), so nothing renders for a mode the editor is not in. One vocabulary, four doors:
...
2. an internal block-facing context getter that joins the prose render key, so a mode flip re-renders every mounted block; `preview-block` reveals the focused block's markers via a focus-keyed root attribute (CSS only — a per-block focus change never rebuilds the inline DOM);
