...
- **Per-instance enablement.** Shipped, and without the new prop the earlier sketch reserved: the **`plugins` prop is the enablement set**. An editor activates exactly the plugins it lists, meaning their `onEditor` hooks, their kinds and openers (through the `RegistryView` over the global definitions), their global commands, and their paste transforms. A plugin another editor installed but this one did not list attaches nothing here, and its kinds resolve no component and degrade to the raw-editable fallback. An editor mounted with no `plugins` prop activates everything installed in the process; that is the documented default, not a leak. One boundary is worth stating plainly: the **initial** parse reads the global grammar, so an unlisted plugin's syntax still parses to its kind and then degrades on render, and the instance grammar governs every reparse after that. Two more are open on the ledger: an unlisted plugin's chord is still consumed in an editor that never listed it (#265), and inline syntax, inline widget kinds, directives, and block completers still reach every editor (#266). The harness-only `__registryEnablement` door stays, because the filtered-view proof wants a predicate it can vary without varying the install.
