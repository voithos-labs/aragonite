...
- **Path-addressed caret placement.** `rects.scrollTo(path, opts?)` ships the navigation half: reveal (mount) the target, then scroll the viewport to it. Placing the caret _at_ a path is deliberately not exposed. It arrives additively as a sibling read on the same surface (a future `focus(path, offset)` that descends to the leaf and sets the selection), never a second navigation object, per the rect-surface growth rule above. Deferred because a caret write crosses into the selection and commit model (cross-block normalization, the deep-vs-shallow cursor contract, undo-visible focus), so its shape firms up with a consumer that needs programmatic caret control rather than being guessed alongside the read-and-scroll surface.
