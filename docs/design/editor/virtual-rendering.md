...
- **Off-window transparency depends on the eager inline cache.** Cross-block vertical navigation must skip vertically-transparent blocks (image-only paragraphs, thematic breaks) even when they are off-window and unmounted. The test reads the block's inline-content cache directly from the CST, which works because the cache is maintained on every prose node regardless of mount state. A future move to lazily-computed inline content would have to keep that cache answerable for off-window nodes, or route these candidates through reveal.
