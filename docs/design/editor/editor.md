...
- **The render path must not read `node.inlineContent`.** Prose blocks rebuild the styled DOM by re-parsing `node.raw` locally on each render; the reactive read set of a render effect is `node.raw` plus its closure inputs, never `node.inlineContent`. The original incident was a render effect that both read and wrote the cache — write-during-read closed the loop and Svelte 5's ownership tracking corrupted keyed `{#each}` index assignments after `splitBlock`. With reads removed from the render path, `inlineContent` becomes a passive cache: structural mutations may refresh it (commit primitives, the `parseAllInlineContent` helper, and the routine-typing leaf-update path all do), and non-render consumers — event handlers, exported block-component methods, click-snap — may read the latest value without forming a read/write cycle.
