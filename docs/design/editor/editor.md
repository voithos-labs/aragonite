...
| FencedCode              | `fencedCode`              | Code editing surface (contenteditable with live `.code-tok-*` span rendering via `code-surface/` + highlight.js). Participating sticky-column block. Fences rendered as dimmed `.md-marker` spans; info string styled with `.md-lang`. |
