...
**Capture and consumption are split.** The source block (the one holding focus when a vertical arrow is pressed) captures `x` via `stickyColumn.capture`. The focus dispatcher (`editor-actions/focus.ts` and `container-state/focus-dispatch.ts`) reads `stickyColumn.get()` at cross-block transitions, null-checks, and either calls `focusAtColumn(x, from)` with the finite `x` or falls back to `focus(0)` / `focus(CURSOR_END)`. Participating surfaces implement `focusAtColumn(x, from)` as a pure receiver; they never see `null`. The authoritative statement lives in the `contenteditable/sticky-column.ts` header.
