...
- A scope **self-activates**: it windows only when its estimated content height exceeds a viewport budget. Small containers never activate and render exactly as a non-windowed editor would. Activation uses **hysteresis** — separate activate and deactivate watermarks — so a list hovering at the budget does not thrash between windowed and plain on every edit.
- Activation is **gated above the watermark** by whether the editor owns a scrollport at all. In host-scroll mode an ancestor of the embedder scrolls the editor, and a scope cannot window against a container it neither owns nor may write — so windowing is disabled outright rather than approximated, and every scope reaches the same never-active result the below-watermark branch already produces. The mode forfeits the O(viewport) bound and mounts the whole document; it is scoped to the small embedded documents that trade is acceptable for. One decision point stays: the gate is read where activation is decided, not substituted at a consumer, so `active` never disagrees with what renders.
- There is one scroll container: the editor root. An activated nested scope computes its own slice by mapping the root's scroll position into its own pixel range, measured against the editor viewport.

```
scroll    → editor scroll position → height model maps to [start, end) per active scope
...
**Preamble chrome above the list.** A host may render its own chrome inside the scroll container, above the block list — a title, a properties panel. The slice math needs no case for it: a scope measures its list's live offset within the scroll content and takes its viewport as the list's intersection with the scrollport, so a preamble shrinks the scope's window rather than displacing it. What a preamble does need is the correction, because its height is outside the height model: a slot that grows or shrinks routes its delta through the same scroll compensation a measured-in block does. The compensation is inert in host-scroll mode — the scroll belongs to the host page, and an embedded editor does not write an ancestor's scroll position.
...
A reveal that must also _hold_ what it revealed — a navigation, a search jump, a consumer restore — claims the **reveal anchor**. While a claim is live the root scope re-asserts the target's position on every measure pass instead of holding the top-of-viewport block, which is what keeps a target past undecoded images from being clamped off-screen as they collapse the document. The slot holds one target at its full path, with per-call ownership: a claimant may release only the pin it still holds, a superseded reveal stops refining rather than fighting the newer one, and any user-intent gesture in the document outranks all of them. Nested scopes never claim it — their corrections would fight over one scroll position.
