...
- **Measured cache, keyed by stable id.** Once a block mounts, its real height is measured and cached by stable block id, not by index, so split/merge index shifts and undo (which restores ids with the snapshot) do not invalidate it. Measured always supersedes estimate; stale entries self-correct on the next mount. Its lifetime is the document: a whole-document replacement through the `source` prop drops the whole cache, since ids never recur and every entry would key a block that cannot come back.
...
Narrowing the content column re-wraps prose, so cached heights computed at the old width are stale. On a width change the root clears the measured cache, and each scope falls back to width-parameterized estimates, re-measures its currently-mounted blocks immediately, and rebuilds its model, all inside an anchor correction so the viewport holds through the reflow. A height-only resize spares the **measured cache** (prose re-wraps on width, not height), but not the slice: the window's extent is derived from the scrollport's height, so a height change recomputes it.
