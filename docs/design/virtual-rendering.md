...
- Every kind's guess is its descriptor's `estimateHeight`, `(node, env) => px`, where `env` carries the width and the type metrics. Built-ins pick one of the shapes in `src/lib/schema/height-estimates.ts` (wrapped prose, one line, source lines, a container by its children), a plugin that declares none gets the container or the prose shape, and a plugin with a tricky height supplies its own. The bundled mermaid plugin's is the whole story in one line:

  ```ts
  estimateHeight: () => 320, // the skeleton's fixed height; the real one replaces it on mount
  ```

  The pecking order: a collapsed container is guessed at one chrome row no matter what (its body never paints), then the kind's `estimateHeight`, and a real measurement beats everything.
...
- For things that grow after mounting (e.g. an image decoding, a font swapping in, a lazy embed), those have to be accounted for lest we want to cause the dreaded slide; so every rendered block reports its size changes and windowing corrects for that. A block starts watching its size one frame after it mounts: a correction can mount a block while the browser is still handing out size reports, and a watch begun then is skipped and logged as a ResizeObserver loop error. Nothing is lost, since the mount's own height comes from the batch and the first report carries whatever changed since.
...
| VR-15 | A structural rebuild (a split, a paste, an undo) keeps a surviving block's measured height. Only a width or type-scale change reseeds the model from the height estimator, so a measured cache dropped earlier by a mode flip cannot turn a whole document back into estimates and scroll the reader by the difference.                                                                                                       |
