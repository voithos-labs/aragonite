...
| VR-15 | A structural rebuild (a split, a paste, an undo) keeps a surviving block's measured height. Only a width or type-scale change reseeds the model from the oracle, so a measured cache dropped earlier by a mode flip cannot turn a whole document back into estimates and scroll the reader by the difference. |
