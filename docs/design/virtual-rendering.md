...
**The windowing wiring is extensible; the height model is not.** A plugin container inherits windowing by naming only its real variation (see § Recursive Windowing), but `estimate()` is a closed switch over the built-in kinds with a prose char-wrap default — there is no per-kind estimate hook. A plugin kind therefore falls into that default arm and is estimated as if it were prose, and self-corrects on first measure. The one declarative influence a plugin kind has is the collapse probe on its reserved-chrome declaration: a collapsed container is estimated at one chrome row, because that is all it mounts.
...
**The live limitation:** a rendered block whose height is genuinely unpredictable from its source — a Mermaid diagram, a KaTeX render — has no better estimate available to it. The height model is closed (see § Height Oracle and Model), so such a kind falls into the prose char-wrap default arm and is estimated as if its source were prose. It reconciles through the ResizeObserver + anchor-correction path on first render and self-heals thereafter via the measured cache, so the viewport stays content-stable — but the pre-measure estimate for these kinds is a guess, and the thumb drift before they first mount is correspondingly larger. A per-kind estimate hook (a fixed-height skeleton placeholder would be the natural shape: exact while the skeleton shows, so it adds zero scroll-time error) would close this; it is not built.
