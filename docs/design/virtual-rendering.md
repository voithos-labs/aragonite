...
| VR-14 | A window lags its children by one pass after a change. The slice clamps it to the children, even while windowing is off, so swapping a one-block document for a huge one never mounts every block for that pass; a window whose end comes before its start collapses to empty.                                          |
