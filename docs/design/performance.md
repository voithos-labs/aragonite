...
- **Typing inside a large container.** A container's `raw` holds its full outer source, so the rebuild that keeps it honest used to re-read every child on every keystroke inside it. The variable is the container's CHILD COUNT, not its bytes and not where the caret sits: CPU-profiled on 2026-08-26, a keystroke on the head child and one twenty children deep cost the same, and the dominant term was the `$state` proxy's read trap (about 25,000 of them per keystroke on a 1MB list), not the concatenation those reads fed. Production build at 1MB, before the fix: **17.7 ms** per keystroke for a single giant list, **16.0 ms** for a giant blockquote, against ~2.6 ms, the recorded DEV-build baseline, for the same fixtures typed into a paragraph ahead of the container: the control has no production reading of its own until the re-bless, so read the gap as a shape rather than a ratio. The **~52 ms** this doc carried until then was a dev-build number: roughly 30 ms of Svelte's dev-only subtree relabelling over roughly 21 ms of editor work, a cost the gate and the README charts paid and no user ever did. The rebuild now rewrites the changed child's region alone (§ 9 of `editor.md`), reading one child instead of all of them. The axis belongs to the container `raw` contract rather than to any one feature: it measured the same with the container kind re-derivation disabled. It went unrecorded until 2026-07-28 for an embarrassing reason, every latency row having prepended a paragraph, so the caret always had a top-level home and never sat where the cost was; the `giant-single-{list,blockquote}-interior` rows now type inside the container and gate it. What remains on this axis is the tail join below. Blessed 2026-08-26 on the production build, after the child-spans rebuild: 3.3 ms (list) and 3.0 ms (blockquote) at 1 MB, and 5.4 ms for the 10 MB list, against 17.7 / 16.0 ms before.
...
- **The lower join at a large container's tail.** A write in a large container's last child, with a block following the container, pays the seam ask over the container's own bytes: ~34–37 ms per keystroke at ~650KB, against ~0.5 ms with the ask off. It is the interior-typing axis's twin at the other end and a different cost: the ask parses bytes where the rebuild read children, so the child spans do nothing for it. The gated fixtures are single top-level blocks, so no ceiling sees it. Not gated, tracked as #182.
...
| --------------------------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `e2e` (keystroke latency)               | **Gated**       | `npm run perf:check`, over a production build — every renderable shape at ≤1MB, the 10MB keystroke, the container-interior rows, and two live-mode 1MB rows ceilinged at their source twins |
