...
| `flat-prose-10MB-structural` (a top-level Enter and Backspace)     | **Gated**       | `npm run perf:check`, the same ceiling formula over the p50 per edit. A split or merge at the top level rebuilds the whole windowing model, which no typed character does, so on a large flat document this row pins that rebuild at pointer work per block, never work per byte |
...
| `arrival-widget-only` (an arrow into a paragraph of widgets alone) | **Report-only** | Nothing; printed by `npm run perf:e2e`, skipped by the gate. The one row that times an arrow rather than a keystroke                                                                                                                                                             |
...
**Keystroke-latency attribution.** The dominant steady-state keystroke cost is framework reactive-flush work proportional to the number of MOUNTED components. It sits outside every boundary where the editor's own code hands off to the next piece. Only the edited block re-renders, and parse, inline refresh, ancestry rebuild, and snapshot each run about one unit, yet the flush scales linearly with mounted block count. Which is the annoying part: no amount of tuning inside the seams touches it. The only lever that turns O(mounted) into O(viewport) is to genuinely unmount off-screen blocks. Hence virtual rendering.
