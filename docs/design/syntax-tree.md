...
The third is the line under an HTML block that a move takes away. An HTML block of the kinds that close at a blank line reads every line under it as its own text, a quote or a list included, so when the block holding that line moves elsewhere, the move writes a blank line under the HTML block and the blocks below keep their kind.
