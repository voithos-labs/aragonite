...
One separator has no splice to derive it from: a list whose first item is empty, right under a paragraph. A content-less list marker can't interrupt a paragraph (GFM § 5.2, list items), so an item reading `- x` followed by an indented bare `- ` reloads as `x` with a `-` underline, which is a setext heading, and `para` over an emptied `- ` reloads the same way. The marker is the only evidence the item ever existed, and the merge that absorbs boundary lines has nothing to fold it into. Every path that reaches the shape (the Enter-then-Tab nesting move, emptying the first item, a replace or splice that lands such a list) settles a blank line above the list, which is why nesting an empty item leaves a loose list (loose: a list whose items render with paragraph spacing, because a blank line sits inside it).
