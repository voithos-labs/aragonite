...
- **Container blocks** — hold children: blockquote, list, listItem, table, tableRow, and any plugin- or directive-authored container.
...
That dispatch is a per-keystroke write-amplification of ≈ chain-depth ÷ 2 (each ancestor re-materializes everything from its level inward). `container-raw.bench.ts` (deep-nesting axis) measures it, and the variable is bytes re-materialized, not depth: ≈2 µs/KB holds across the whole axis, so the same depth 8 costs 4.9 µs when the containers are small and 3.5 ms at 50 KB/level, while an adversarial depth 16 × 100 KB reaches 24.6 ms (2026-08-20, recorded machine). The Σ over levels is where the depth²·bytes shape comes from; real documents keep the byte term tiny, so a nested keystroke stays in the viewport-bounded floor class. The redundancy was weighed and kept — `container-raw.bench.ts` is the standing evidence.
...
The CST was designed around three phases. Two shipped; the third was evaluated and rejected. It's recorded here because "why isn't the inline tree authoritative?" is the first question a rich-text-editor person asks, and the answer is load-bearing.
