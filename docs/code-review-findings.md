...
reference captured *before* the commit reads a stale value.
...
- [ ] **Important — `editor-actions/table-context.ts:201,203` (`deleteRow` afterTick).**
  ...
  reference, so `targetRow = Math.min(rowIdx, newRowCount - 1)` uses the *pre-delete* count.
...
- [ ] **Important — `editor-actions/table-context.ts:219` (`deleteColumn` afterTick).** Same
  ...
  *(Contrast: `insertRow`/`moveRow` afterTicks use pre-computed indices and are correct.)*
...
- [ ] **Minor — `selection/widget-range-paint.ts:68-76` (`safeIntersects` fallback).** The
  `compareBoundaryPoints` fallback (for runtimes lacking `Range.intersectsNode`) is inverted:
  it returns `true` only when ranges *don't* intersect. Production-dead (modern browsers and
  jsdom both ship `intersectsNode`), but wrong. Fix: `END_TO_START > 0 && START_TO_END < 0`.
...
*branding* tokens that leak the old app name, one of them in the **public API**.
...
- [ ] **Minor — `docs/code-style.md:3`, `docs/commit-conventions.md:3`.** Say "Limestone-specific"
  ...
  *(Test fixtures using "# Limestone" as document content are harmless and intentionally left.)*
...
- [ ] **Important — no test for delete-last-row / delete-last-column focus landing.**
  ...
  and delete a *non-last* row. The Theme-1 bug (focus on a non-existent cell — possibly a throw)
...
- [ ] **Minor — `safeIntersects` fallback is never exercised** (Theme 4). Add a test that stubs
  `Range.intersectsNode` away to cover the (currently inverted) fallback.
...
- [ ] **A2 — next/prev doesn't scroll a mounted-but-offscreen match.** `revealPath`→
  `revealChildOrWait` only scrolls when the target block is *unmounted* (windowed out); a match in
  a mounted-but-scrolled-out block updates count+highlight without scrolling. Fix: after reveal
  resolves, `scrollIntoView({block:'nearest'})` on the active match's block. *(Touches reveal
  logic — verify against windowing.)*
...
- **`core/parsers/list.ts:68-69`** — correct. `i = j + 1` advances past blanks *and* the content
  line; `joinRaw(itemStartIndex, i)` includes the blanks, so round-trip is preserved.
- **`core/inline/emphasis.ts:276-277`** — correct. Marker offsets are read *before* the
...
- **`components/image/ImageOverlayHost.svelte:64`** — not the claimed bug. `syncOverlayToWidget`
  invokes `getOverlay()` *synchronously* during the effect, so Svelte 5 tracks `imageOverlayEl`
...
- **`components/blocks/list/ListItemBlock.svelte:135` (isEmptyItem)** — not data-loss.
  `exitListAtItem`→`buildExitReplacement` *relocates* trailing children; the shallow check is
