// The `/` demo document. Array-joined because the fences and inline code carry backticks;
// the block-math backslashes are doubled for the JS string, not the Markdown.
export const SHOWCASE_DOCUMENT = [
	...
	'      - Four levels. So, no.',
	'',
	'A list this size just renders. One with ten thousand items windows its own children, meaning only the ones on screen get mounted, and yes, the test suite builds one.',
	'',
	'## Tables, one cell at a time',
	'',
	"Each cell is its own little editing surface. `Tab` hops to the next cell and `Enter` drops to the row below (try it). One heads-up: the first time you edit inside a table, its padding and delimiter row get tidied to the standard spelling. Quotes and lists do the same to their own punctuation. That's the one time the editor writes a byte you didn't.",
	'',
	...
	'',
	'You opened it. Of course you did. A details block holds real Markdown in real blocks, so this list is as editable as everything else:',
	'',
