...
// Enter at the end of a lone `$$` completes a fence pair whose two-line form could never be typed
// into directly, the plugin counterpart of the built-in table's completion. The source bytes are
// the reference, and where the caret landed is read by typing rather than by asking for it.
// Requirements: e2e/requirements/plugins/latex-typed-formation.md.

const COMPLETED = '$$\n\n$$\n';

/** Type `text` at the end of block `index`. A lone `$$` completes as it lands, from the on-type
 *  rule, so Enter is only pressed here where the line is not `$$`. */
...
test.describe('block math: typed formation', () => {
	...
	// as it lands, the way a typed ``` is a fence at once, with the caret already on the body.
	...
	// Mid-line the pair is prose: only a line that is nothing but `$$` opens anything.
	...
	// The Enter path is still there for a `$$` line that arrived without being typed, from a
	// loaded document or a paste, and it builds the same shape.
	...
	// An open source commits on blur, so the block above is both what takes the blur and the proof
	// that the completion replaced only its own block.
	...
	test('one undo after the blur restores the paragraph byte-for-byte', async ({ page }) => {
		...
		// The undo snapshot holds where the caret was, not where the completion sent it.
	...
	test('an opener carrying body text falls through to the ordinary split', async ({ page }) => {
		// Loaded, not typed: the on-type rule would take the `$$` before the ` x` arrived.
	...
	// Refusing is only worth asserting if the same key works once the mode changes; otherwise a
	// key that never reached the editor would pass too. The completion afterwards is that proof.
...
// Recognizing the syntax needs the plugin installed, and so does completing it: the editor harness
// loads no plugins, so the same keystrokes must leave plain GFM's own shape.
