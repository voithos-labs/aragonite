...
/**
 * An inline syntax handler that turns `![[path|width]]` into the built-in `image` node. Every read
 * path treats it as an image, which is the point, and the write paths must not: without
 * `rewriteImage` a resize replaces the embed with a plain inline image. Only the real gesture
 * shows it, because a round-trip check cannot: the document round-trips perfectly, as something
 * else.
 */
