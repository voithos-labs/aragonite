...
// Shared probe surface for the two inline-reveal command suites (the fold seam and Enter's block
// meaning). Both drive the same gestures against the `math` seed, so the reveal entry and the
// byte-at-a-time source edit live here rather than being carried twice.
