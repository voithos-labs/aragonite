...
makes the source the only thing
that can check them, and makes a silent divergence invisible on screen by
...
family nets to identity and the session's round-trip and nested-state checks
run over every step.
