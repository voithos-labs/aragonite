...
Two loaded-ops sessions on the plugins route, over the render-primary block and
...
container. Their mount/unmount churn and reveal↔source swaps are exactly the
...
exists to catch. Both sessions re-check all three oracles after every move, with a
...
- no console, page, or structured editor error fires across either session,
