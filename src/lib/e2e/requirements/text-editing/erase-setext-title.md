...
with real keys; the source, the block kind and the rendered dividers are what each scenario checks.
...
- assertions read the source and the block kind through the bridge, and count the dividers on the page
