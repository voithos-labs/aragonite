...
editor 2 = `[calloutPlugin(), detailsPlugin()]`, both on a seed holding a `:::note`
...
- the shared plugin renders in both editors: `callout` resolves `:::note` to a `note` container (child 0 `note-title`) in editor 1 _and_ editor 2, proving a process-global registration serves a later mount

## Edge cases

- an already-parsed document does not re-parse against a later grammar: editor 1 — parsed before detailsPlugin existed — keeps its `<details>` seed as the built-in `htmlBlock` (never `details`), and its `:::note` stays `note`; the only invariant console fire is the one expected `[invariant:late-opener-registration]` (details' opener registered after editor 1 consumed the grammar)
