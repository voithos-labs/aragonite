...
- Every presentation-mode pill lies fully inside the viewport, and a tap on `live`, the
  far end of the group and the one a strip that cannot wrap loses first, switches the editor
  to that mode.
- The find bar opens fully inside the viewport, and a tap on its field followed by typed
  text reports matches.
- With the drag-handles toggle on, the handles are opaque with nothing hovering them, and a
  tap lands on the handle rather than falling through to the editor.
...
- Width: `playwright.config.ts` pins one 1280 viewport, and the handful of specs that
  ...
  out in a column narrower than its own controls, so a control off the right edge, a bar off
  the left one and a block panning the document were all invisible to the suite.
- Touch: no spec had ever run with `hasTouch`, so a control shown only on `:hover`
  read as present in every run: `toBeVisible()` passes on an `opacity: 0` element, and
  only a hit test catches the `pointer-events: none` half.
- Target size: nothing in the suite had ever measured a control's box against a minimum,
  on any pointer, so controls sized for a mouse cursor were never asked what a thumb needs.
- Target-size census: the first pass matched `.showcase-header button`, and the header's two
  links are anchors, so the sweep closed the class everywhere except at the elements its
