...
presentation modes. This is not a behavior test: what it checks is what the published image shows,
so the bullets below are the picture's contract.

## Light and dark

- one strip per page theme, so the README's `<picture>` can pair them like the charts do
- the editor keeps its default theme in both; what changes is the page around the panels
...
- the preview-inline panel holds the caret inside the bold word, so that construct's markers show
  while the rest of the document's stay hidden
- the live panel holds the caret in the same word, and its markers stay hidden anyway: the caret in
  the same place in both panels is the strip's whole argument
...
- the caret never lands (the word is off screen): the run fails rather than publishing a panel that
  shows the wrong markers for its mode
