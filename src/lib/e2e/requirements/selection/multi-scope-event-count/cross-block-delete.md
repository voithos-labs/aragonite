...
Pinned: the spec's identity test drives this scenario with real gestures, and `computeScopeDescriptor`'s mixed-depth branch maps the survivor to the start item's id (fixed pre-0.6 in `af42604a6`, which unblocked the spec's fixme). Issue #74 was filed from this paragraph while it still read Known-status.
