...
the work: the shell decides whether to answer a click on its own territory, the editor
decides where the caret lands. It answers a raw viewport point with no click, no target
and no gesture in front of it, and reports whether a caret landed.

The landing uses the dead-space click's own walk, so this file asserts what a consumer
sees through the public method; how the gesture tells cases apart (pairing a press with
its release, the drag release, a live native range) is in `dead-space-click.md`.
...
- A point over a block's own text lands there. The method is not dead-space-only; the
  shell's decision to call it is the only check.
...
- The consumer defect that motivated the method (a journal's dead strip beside the
  text) was a bug in the host's CSS, not in the editor: the shell put the reading
  column's inset on an ancestor of the editor element, so margin clicks never reached
  anything the editor could handle. No editor test could have caught it, which is why
  the fix is half documentation (the host-scroll CSS contract) and half this method,
  for the frame that genuinely is the shell's.
