...
across blocks; the word or block you clicked on is always in it. The browser owns none of this:
...
- triple-click past the end of a line, on no glyph: the paragraph, and it stays selected after
  the release
  - Miss-analysis: every multi-click test clicked on a glyph, where the browser's release
    places no caret; none clicked past a line's end, the one place the release does place one,
    so the range that vanished on release was never on screen in a spec
- triple-click in a quote's gutter (the quote's own box, left of its text): the quoted
  paragraph, the leaf nearest the click
- double-click on a list item's `- ` marker (a non-editable widget inside the editable text):
  the word beside it; the drag handle that appears in the gutter on hover is a control and
  stays out of the priority order
