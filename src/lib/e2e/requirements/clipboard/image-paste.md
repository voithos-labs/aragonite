...
paste started from. Without the hook, an image-bearing paste behaves as it does
anywhere else: the native default is prevented and the clipboard's `text/plain` (if
any) is pasted.

Replacing a **multi-block** selection is its own concern, covered in
`image-paste-cross-block.md`.

**Playwright exception:** a real image cannot be written to the system clipboard from
a spec, because `navigator.clipboard.write` with an image blob is not scriptable under
the test runner. These specs construct a `DataTransfer` carrying a `File` and dispatch a
...
- The same insertion works in the other editable areas (a table cell keeps the
  markdown inside that cell, a code block takes it as literal source) and the tree
  still matches a reparse in both.
...
- Hook rejects on the first of two images: the error is emitted and the second
  image's markdown still lands, so one failed import does not abort the paste.

## Without the hook

- No `onPasteImage` installed, clipboard carries an image file and `text/plain`:
  the text is pasted, exactly as it is when no hook exists.
