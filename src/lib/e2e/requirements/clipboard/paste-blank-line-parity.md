...
- Pasting `"one\n\ntwo"` (clipboard) → 2 DOM blocks, the same count loading those bytes gives: a lone blank line separates and creates nothing.
...
- Pasting multi-block content into the middle of a non-empty paragraph lands the caret at the end of the pasted content, not on the trailing residue that sat after the caret. Typing immediately after the paste appends to the pasted content (end-of-block pastes, which have no residue, are unchanged). This holds when the residue reads as a continuation of the last pasted block and the fix-up after the splice folds the two into one, where "end of the pasted content" is an offset inside that folded block rather than its end.
