...
- Type `\` followed by every escapable punctuation character `!"#$%&'()*+,-./:;<=>?@[\]^_\`{|}~`: each produces a styled escape (markers visible, escaped character visible) and round-trips intact.

## Edge cases

- Type `\\*`: the first `\` escapes the second `\`, leaving `*` as a normal delimiter. If followed by `foo\\*`, emphasis forms across `\\*foo\\*` (because each pair `\\` is a single escape and the `*` characters are unescaped delimiters).
...
The following scenarios are exercised at the unit-test level (`src/lib/test/core/inline/escapes.test.ts`) rather than via E2E, since they are parser-internal contracts not visible as user-facing DOM behavior:
...
- `\` followed by `\n` left as text for the hard-line-break post-pass to handle (`post-process.ts`).
