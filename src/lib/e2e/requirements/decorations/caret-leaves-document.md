...
So every gesture that ends the document caret has to reach that event, including the
...
The image overlay portal is the only sign of widget-selected state visible from outside,
so it is what "the caret left for the widget" asserts against.

## Happy paths

- click into a repeated word, then click an image widget in the same document: the widget
