...
Live mode paints no delimiter, so the keystrokes that open, close and leave an inline construct
are answered by two pieces of code together: the auto-pair handler (`delimiter-autopair.ts`),
which decides what a typed delimiter writes and steps the caret past a closer, and the rule for
where typed bytes go (`edge-seat.ts`), which decides which side of a hidden run the next byte
lands on. These scenarios drive the two as one user gesture on `/test/editor` via
`?presentationMode=live`, with real clicks and keys; the source is what each scenario checks
against, since the screen cannot show which side of a hidden run a byte took.
...
- a closer typed by hand in a cell puts the next byte outside the construct

## Miss-analysis

- Every row for where typed bytes go started from a construct already in the document and arrived
  at its edge by arrow or click, so no row typed a construct to completion and kept typing: the
  caret a commit puts past a hidden closer (raw 12 of `[ab](u) text`) resolved to the position
  after the hidden span, which Chromium moves back upstream, and the byte landed inside the link
  text.
- The auto-pair rows always had a counterpart to step over, so none typed the closer that makes
  the construct and then asked which side the next byte takes; the rule "a committed keystroke
