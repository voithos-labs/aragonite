...
`data-focused`, set by the editor's focus tracker on the focused block's host. It
is CSS only, with no branch in the render path, so the marker DOM stays intact
(offsets survive) and the caret's DOM anchor survives the markers being shown.
Driven on `/test/editor` via the header "Block preview" toggle (a real click) and
`?presentationMode=preview-block`.

Containment rule: only the single focused leaf shows source. A container's own
frame (a blockquote border, a directive gutter) never toggles; a focused list item
shows its own marker prefix as source while sibling items stay rendered.
...
- clicking into a block shows that block's markers (it becomes focused) and
...
- containment: focusing one list item shows its marker prefix as source with no
...
- click into an unfocused marker-bearing paragraph mid-word: the caret lands at
  ...
  block's markers show
- arrow-key traversal from one block to the next switches marker visibility
  cleanly: the block just left hides its markers, the block just entered shows
  its own
