...
image must not collapse to a zero-width sliver painting nothing: the highlight
covers the widget's box, so the user can see where the match lives.
...
- Searching text that appears only in the image's url paints the same visible
  highlight over the widget, though the match exists nowhere in the on-screen text.
