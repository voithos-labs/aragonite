...
- axisT first edit: the full instrument profile of the first keystroke after load. The one row with a limit: block renders ≤ 50, since a document-wide fan-out reads in the tens of thousands.

## Notes

- All rows run serially (timing-sensitive). Dev server, with dev-mode assertions active, so the numbers are an upper bound on production.
