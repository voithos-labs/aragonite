...
Fixture: `/test/flow` — two journal entries in one scroller (1000px of filler above
...
- `rects.scrollTo` on a block far below the fold resolves `true` and the block ends up inside the WINDOW viewport (the ancestor scrolled, not the root).
- `rects.scrollTo` on a path that addresses no block resolves `false`.
- `rects.scrollTo` inside a host that clips rather than scrolls resolves `false`: no scroll can bring the target into view, and a resolved `true` must mean the target is genuinely visible. Measuring against the editor root — which spans the full document height in host mode — would report `true` for a block nothing can reveal.
