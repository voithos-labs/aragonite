...
- Collapse-to-start does not wipe the table body: after Ctrl+Shift+End, ArrowLeft collapses the cross-block selection (waiting for the collapse before typing); the CST row count is unchanged afterward. A pre-fix race left the selection active and the type range-replaced the whole body. (The marker's landing cell is not asserted — under windowing the collapse-to-start caret lands in the off-window focus cell, a separate pre-existing VR defect.)
