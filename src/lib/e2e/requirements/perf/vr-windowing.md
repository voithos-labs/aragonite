...
mounted), backed by spacer elements that preserve the browser's scroll geometry. A
small document renders every block with no spacers, since windowing stays inactive.
The bound holds for flat docs and for a single giant blockquote / list / table container.
...
- Mounted set is size-independent: loading the same shape at two windowing-active sizes mounts a similarly small window for each; the larger doc has many more CST blocks but not more mounted blocks, so the bound is O(viewport), not O(doc).
...
- Small-to-large swap: replacing a one-block document with the multi-thousand-block one mounts no more block hosts during the swap than the final window holds, and the load returns. The one-block document's window is inactive and sized for one block, and must not be read as "mount every block" for the pass before it catches up. Miss-analysis: the spec loaded after the one-block swap but counted only the settled DOM, which hid every block mounting and unmounting in one flush until the extra memory crashed the renderer.
