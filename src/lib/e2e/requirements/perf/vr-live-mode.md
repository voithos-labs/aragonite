# Feature: Virtual rendering, live mode windows like every other presentation mode
...
real wheel gesture over the editor's own scroll container.

Miss-analysis: every windowing scenario loads its fixture in source mode, and
every live-mode scenario loads a document too short to window, so a check that keyed
activation on the mode turned live into an all-mounted document with nothing to fail;
...
- a switch from source into live (a real click on the header's live toggle),
...
- every mounted-set ceiling carries the shared coverage floor: the mounted band
  reaches both edges of the scroll container, before and after the scroll
