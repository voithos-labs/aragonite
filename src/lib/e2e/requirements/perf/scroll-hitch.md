...
and windowing mounts and unmounts them as the user scrolls, so this is where a
...
- per tick: block hosts mounted and unmounted (counted from DOM mutations under
  the editor), programmatic `scrollTop` writes (the scroll correction, counted at
