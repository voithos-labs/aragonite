...
- Image that _succeeds_ with no intrinsic size (zero-dimension SVG) gets the same placeholder from the load alone: no edit, no mode switch, and the widget's box has a real size
