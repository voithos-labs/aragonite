...
- Fix: `blockRefs` is a plain array (no `$state`); `BlockList` accepts `setRef`/`getRef` callbacks instead of `bind:blockRefs`. Reads are synchronous from `focus.ts` / `block-edit.ts` — no reactive subscriber needs the array.
