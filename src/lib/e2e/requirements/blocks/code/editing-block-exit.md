...
- Backspace immediately after opener fence is a no-op: caret at the start of the body's first column (just past the opener's `\n`) — Backspace must not delete the boundary newline (which would fuse the body into the opener line)
