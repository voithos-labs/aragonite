...
- GH #438: the shape property skipped exactly this shape, and its harness rebuilt the list without the blank line typing writes, so the find bar's replace, which rebuilt the same way, was never checked; convergence passed because the parser read the heading the bytes spelled.
