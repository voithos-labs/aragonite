...
enters it (selecting an image, opening the source of a reveal-capable kind), the next moves on,
and ArrowDown reads it the same way. Both vertical directions must agree, or a run of ArrowUps followed by the
same number of ArrowDowns leaves the caret somewhere else.
