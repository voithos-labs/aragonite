 ...
 * Character width has no CSS number to derive from, so the set is mirrored by hand at one
 * scale. The editor's type scale is font-relative, so consumers scale these by the root's
 * live computed font size rather than reading them raw (`Editor.svelte` does, for the
 * oracle — an estimate calibrated for one scale can miss the windowing watermark entirely).
 */
