 ...
 * Input is not mutated; returned blocks are fresh references produced via
 * structured-clone semantics (JSON round-trip is sufficient for CST nodes
 * because they're plain data).
 */
export function unwrapFirstChildFromBlockquote(blockquote: CstNode): CstNode[] {
	...
	const clonedChildren: CstNode[] = JSON.parse(JSON.stringify(blockquote.children));
