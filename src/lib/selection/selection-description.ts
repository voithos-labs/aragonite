 ...
 * this. Pure — fed to the editor-root live region. Counts top-level blocks
 * spanned; a selection nested within one top-level block gets a generic phrase.
 */
...
export function createSelectionDescription(selection: EditorSelection): string {
	...
	// Endpoints inside one top-level block (e.g. two items of the same list) span
	// no top-level blocks — a count would read "1" and mislead, so stay generic.
