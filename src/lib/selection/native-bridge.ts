 ...
 * type. Four responsibilities:
 *   1. Read a native caret/range inside a block element and map to
 *      SelectionPoint via the block's path.
 *   2. Apply a SelectionPoint or single-block range to the native selection.
 *   3. Clear the native selection entirely.
 *   4. Map a viewport coordinate to a character offset inside a block via
 *      caretRangeFromPoint (with caretPositionFromPoint fallback).
 *
