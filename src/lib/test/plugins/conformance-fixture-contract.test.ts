// F9: the conformance kit's fixture contract — the kind at `children[0]`, sentinel appended
// after it — was load-bearing but stated nowhere, so a plugin author met it as a bare
// assertion failure. The message now carries the contract; this holds it to that.
//
...
describe('checkCopyIsRawByteSlice fixture contract', () => {
	...
	it('states the contract when the kind is not the first child', () => {
		expect(() => checkCopyIsRawByteSlice('thematicBreak', 'lead paragraph\n\n---\n')).toThrow(
			/must parse to its kind at children\[0\], with the kit's sentinel block appended after it/
		);
	});
