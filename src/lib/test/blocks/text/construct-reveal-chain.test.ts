// The offset math behind showing markers in preview-inline mode: which constructs the caret
...
describe('constructChainAtOffset', () => {
	...
	it('a boundary shared by adjacent constructs reveals both', () => {
		// `*a*` [0,3) then '`b`' [3,6). Both show: the caret is a raw offset and an
		// insertion lands at it, so there is no boundary winner to pick.
		expect(chainKinds('*a*`b`', 3)).toEqual(['emphasis', 'inlineCode']);
		// Two constructs that really abut resolve the same way at their shared raw
		// offset: in `**a***b*`, strong [0,5) and emphasis [5,8) both show at 5.
	...
	it('a link chains with its nested formatting', () => {
		// `[x **y**](u)`: the link wraps text `x ` and strong [3,8).
	...
	it('non-revealable kinds never join the chain', () => {
		// Escape [0,2) and autolink: one has markers, the other has none, and neither
		// kind shows its markers this way.
