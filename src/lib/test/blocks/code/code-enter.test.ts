...
/** Normal (auto-indent) mode over an LF body; each case names only what it varies. */
const enter = (args: Omit<EnterArgs, 'mode' | 'ending'> & Partial<EnterArgs>) =>
	computeCodeEnter({ mode: 'normal', ending: '\n', ...args });
...
describe('computeCodeEnter — normal mode', () => {
	...
	it('uses the indent of the line containing the selection start, not the end', () => {
		const result = enter({ display: '    foo\nbar', selection: { start: 7, end: 11 } });
		expect(result.newText).toBe('    foo\n    ');
		expect(result.newCursor).toBe(12);
	});

	it('preserves a tab indent', () => {
		const result = enter({ display: '\tcode', selection: { start: 5, end: 5 } });
