...
import { createTextBatch, UNDO_DEBOUNCE_MS } from '$lib/editor/editor-actions/text-batch';
...
describe('text-batch lifecycle', () => {
	...
	it('batch-key change flushes the displaced batch and starts a new one (0.7.7 regression)', () => {
		const { batch, pushSnapshot, emitInput } = harness();
		batch.keystroke(0, 0, 'leaf-a');
		batch.keystroke(0, 1, 'leaf-a');
		batch.keystroke(0, 0, 'leaf-b');
		expect(emitInput).toHaveBeenCalledWith(0, 2);
		expect(pushSnapshot).toHaveBeenCalledTimes(2);
	});

	it('numeric fallback key: a different blockIndex breaks the batch and flushes the displaced one', () => {
		const { batch, pushSnapshot, emitInput } = harness();
		batch.keystroke(0, 0);
		batch.keystroke(1, 0);
		expect(pushSnapshot).toHaveBeenCalledTimes(2);
		// Pins flush-before-repoint: the displaced batch's OWN index is emitted,
