...
// A cross-block merge re-parses the joined raw, and the result is allowed to
// change kind — leaf into CONTAINER. The caret is restored by walking the block
// element at its path, so a caret naming a container walks the whole rendered
// subtree and lands focus on a non-editable wrapper. The caret must name a leaf
// whatever kind the merge produced. Byte assertions ride along: the CST was
// always correct here, so the fix must not move a single byte.
