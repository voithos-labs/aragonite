...
describe('WidgetSelectionState', () => {
	...
	it('select sets a target', () => {
		const s = createWidgetSelectionState({ onSelect: () => {} });
		s.select({ paragraphPath: [0], sourceStart: 5 });
		expect(s.getSelected()).toEqual({ paragraphPath: [0], sourceStart: 5 });
	});
	...
	it('selecting a different widget replaces the previous selection', () => {
		const s = createWidgetSelectionState({ onSelect: () => {} });
		s.select({ paragraphPath: [0], sourceStart: 5 });
		s.select({ paragraphPath: [1], sourceStart: 0 });
		expect(s.getSelected()).toEqual({ paragraphPath: [1], sourceStart: 0 });
	});
	...
	it('isSelected returns true for the matching path+start', () => {
		const s = createWidgetSelectionState({ onSelect: () => {} });
		s.select({ paragraphPath: [0, 1], sourceStart: 12 });
	...
	it('select clones paragraphPath (caller mutation does not leak)', () => {
		const s = createWidgetSelectionState({ onSelect: () => {} });
		const path = [0, 1];
		s.select({ paragraphPath: path, sourceStart: 5 });
		path.push(99);
