...
describe('G1.35 single-node sink', () => {
	...
	it('stays silent through the merge entry points, refused join included', () => {
		...
		expect(mergeWithNext(plural, 0, undefined, undefined, defaultGrammarView).change).toEqual({
			op: 'noop'
		});
		expect(takeDevWarns()).toEqual([]);

		const ordinary = parse('alpha\n\nbeta\n');
		expect(mergeWithNext(ordinary, 0, undefined, undefined, defaultGrammarView).change.op).toBe(
			'replace'
		);
		expect(takeDevWarns()).toEqual([]);
	});
