 ...
 * reads a channel a registered sink silences, taking Svelte's runtime warnings off the gate too
 * once it swallows the call: either way the gate is blind for that whole file. One file pins the
 * arm itself and says so below.
 */
	...
	it('the console.warn spies are exactly the files that pin a warning channel', () => {
		expect(
			...
			'a registered sink takes reporting over, so a console spy sees nothing a dev warning ' +
				'wrote, and one that swallows the call hides every Svelte runtime warning from the ' +
				'gate; a test asserting on one is asserting on silence'
