...
// derived render caches (`inlineContent`), `metadata`, or editor-level fields
// (`ownerEpoch`), and never recurses into `children` (a container's full
// subtree lives in its own `raw`). Corrupting every cache and metadata in the
// tree must leave serialize output byte-identical. Guards the raw-as-truth
// decision against Phase-3 creep where a serializer might start reconstructing
// output from parsed fields.
