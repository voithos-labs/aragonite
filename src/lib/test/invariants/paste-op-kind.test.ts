...
// through the replace-at-parent path, at every depth), while the list/container
// absorb-and-merge strategies emit `paste`. A consumer counting pastes across
// paths must watch both — watching one misses the other. Driven through the live
// `pasteDispatch` so the guard tracks the real routing.
