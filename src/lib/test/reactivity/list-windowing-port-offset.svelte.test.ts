...
const BLOCK_PX = 50;
const COUNT = 100;
/** Page content between the scroll container's content origin and this editor's first block. */
...
describe('windowing against a scrollport the editor sits inside', () => {
	it('reveals to the port coordinate, not the scope-local one', async () => {
		...
		// Dropping the editor's offset within the scroller would land on 2500, a whole 400px band
		// above the target, which is exactly the wrong range.
	...
	it('windows the same band whether the port is the page or an ancestor box', async () => {
		// The same editor, 400px into the content, inside a container whose own box starts at 120:
		// the container's offset has to cancel out entirely, leaving one answer.
	...
	it('maps a scroll into the same slice an editor-owned scrollport would', async () => {
		...
		// Same document, same viewport, containers differing only in where the editor starts.
