<script lang="ts">
	 ...
	 * the end of the document and lands the caret in it, so there is always a way to write below
	 * whatever the last block is (a table, a fence, an equation). A drag that starts on it is the
	 * editor's own drag-select, like one from any other margin (`Editor.svelte` :: `startMarginDrag`).
	 */
