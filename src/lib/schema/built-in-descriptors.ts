/**
 * Built-in block-kind descriptor registrations. Imported once for side effect
 * from the two descriptor-read entry points: `core/inline/index.ts` (the
 * headless parse/inline surface — getContentRange/isProseKind) and
 * `components/built-in-blocks.ts` (the editor mount, which augments the built-in
 * `table` descriptor and so needs it registered first). Split out of
