...
Note that aragonite is a work in progress (as steam users call it: in early access). Its written in typescript and svelte (the superior frontend framework) (one day i might port this to react, one day) (angular users - why are you still on angular?) (what does vue have that svelte and react doesnt have? a lower barrier to entry?), and tested on chromium browsers (chrome and edge) (it might work in safari/firefox, but I did not test them yet). Yes, there are plans to port to different frontend frameworks and test in different browsers. No, not right now, sometime in the future.
...
In our naiveness, we figured making an editor app should be easy. It's not, not by a long shot. And its only two years later, on my fourth iteration of aragonite, that I gained a deep respect for Obsidian; for all its blemishes, it should be respected for what it achieved.
